...
export function buildRolloverPreview(input: {
  ...
  const promotableStudentIds = input.students
    .filter(
      (student) => student.status === "active" && student.class_id && !isTerminalClass(student.classes?.name || ""),
    )
    .map((student) => student.id);
  ...
  const requiredEntryClasses =
    input.schoolType === "primary" ? ["P.1"] : input.schoolType === "secondary" ? ["S.1"] : ["P.1", "S.1"];
...
export function deriveGradeWorkflowStatus(grades: Array<{ status?: string | null }> = []): GradeWorkflowStatus {
  ...
  return input.teachers
    ...
    .map((teacher) => {
      const timetableConflict = input.timetable.find(
        (entry) =>
          entry.teacher_id === teacher.id && entry.day_of_week === dayOfWeek && entry.period_number === input.period,
      );
