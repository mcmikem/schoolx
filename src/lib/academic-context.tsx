...
function resolveTermByDate(now: Date = new Date()): 1 | 2 | 3 {
  ...
  // During a school break, show the next upcoming term instead of falling
  // back to Term 1, which makes late-August and early-September misleading.
  if (month === 5 || month === 8 || month === 9 || month === 12 || month === 1) {
    if (month === 5 && day < 27) return 2;
    if (month === 8 && day > 23) return 3;
    if (month === 9 && day < 16) return 3;
    return 1;
  }
...
export function AcademicProvider({ children }: { children: ReactNode }) {
  ...
  const loadAcademicSettings = useCallback(async () => {
    ...
    try {
      ...
      if (allTerms?.length) {
        matchedTerm = allTerms.find((t) => t.start_date <= todayStr && t.end_date >= todayStr);
      }
