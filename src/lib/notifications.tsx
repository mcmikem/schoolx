...
export function NotificationsProvider({ children, schoolId }: { children: ReactNode; schoolId?: string }) {
    ...
    try {
      ...
      try {
        ...
        if (todayPayments && todayPayments.length > 0) {
          const total = todayPayments.reduce((sum, p) => sum + Number(p.amount_paid), 0)
          generatedNotifications.push({
            id: 'payments-today',
            type: 'payment',
      ...
      try {
        ...
        if (students && students.length > 0) {
          ...
          const totalExpected = (feeStructure || []).reduce((sum, f) => sum + Number(f.amount || 0), 0) * students.length
...
export function useToast() {
  ...
  return {
    ...context,
    success: (message: string) => context.showToast({ type: 'success', message }),
    error: (message: string) => context.showToast({ type: 'error', message }),
    warning: (message: string) => context.showToast({ type: 'warning', message }),
  ...
  return (
    <div style={{
      ...
      {toasts.map(toast => (
        ...
        >
          <span className="material-symbols-outlined" style={{ fontSize: 20, color: getColor(toast.type) }}>
            {getIcon(toast.type)}
          </span>
          <span style={{ flex: 1, fontSize: 14, color: 'var(--t1)' }}>{toast.message}</span>
          <button
            onClick={() => onRemove(toast.id)}
            style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}
          >
            <span className="material-symbols-outlined" style={{ fontSize: 18, color: 'var(--t4)' }}>close</span>
          </button>
        </div>
      ))}
      <style>{`
