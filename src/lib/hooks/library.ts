    ...
    async function fetchData() {
      ...
      try {
        ...
        if (booksRes.error && booksRes.error.code !== '42P01' && booksRes.error.code !== '42501' && booksRes.error.code !== 'PGRST116') {
          logger.error('Error fetching library books:', booksRes.error.message)
        }
        if (issuesRes.error && issuesRes.error.code !== '42P01' && issuesRes.error.code !== '42501' && issuesRes.error.code !== 'PGRST116') {
          logger.error('Error fetching library issues:', issuesRes.error.message)
        }
        setBooks(booksRes.data || [])
        setIssues(issuesRes.data || [])
      } catch (err) {
        logger.error('Error fetching library data:', err instanceof Error ? err.message : 'unknown')
      } finally {
        setLoading(false)
      }
    }
