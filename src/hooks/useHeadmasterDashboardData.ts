...
export function useHeadmasterDashboardData(schoolId?: string, currentTerm?: number, academicYear?: string, students: any[] = []) {
    ...
    async function fetchData() {
      ...
      try {
        ...
        const [
          ...
          supabase.from('grades').select('student_id, score, term, academic_year').eq('term', currentTerm || 1).eq('academic_year', academicYear || '2026'),
          supabase.from('attendance').select('student_id, status, date').order('date', { ascending: false }),
        ...
        // Fee totals
        let todayTotal = 0, weekTotal = 0, termTotal = 0
        ...
        setData({
          ...
          smsStats: { sentToday: messagesData?.length || 0, deliveryRate: messagesData?.length ? Math.round((messagesData.filter(m => m.status === 'delivered').length / messagesData.length) * 100) : 0 },
          ...
          lowAttendanceClasses: Object.values(attendanceByClass).filter(c => c.total > 0 && (c.present / c.total) < 0.7).length,
