...
import {
  ...
  validatePassword,
  validateDigitizationFee,
  validatePlanUpgrade,
} from "@/lib/server/marketer-logic";
import { computeTrialExpired } from "@/lib/auth-context-types";
import type { School } from "@/types";

const calculateSummary = calculateEarningsSummary;
...
describe("Marketer – Trial Expiry", () => {
  const school = (overrides: Record<string, unknown>) => overrides as unknown as School;

  it("identifies an expired trial correctly", () => {
    const pastDate = new Date(Date.now() - 86400000).toISOString();
    const expired = computeTrialExpired(school({ subscription_status: "trial", trial_ends_at: pastDate }));
    expect(expired).toBe(true);
  });

  it("does not flag active trial as expired", () => {
    const futureDate = new Date(Date.now() + 86400000).toISOString();
    const active = computeTrialExpired(school({ subscription_status: "trial", trial_ends_at: futureDate }));
    expect(active).toBe(false);
  });

  it("does not flag tester accounts as expired", () => {
    const pastDate = new Date(Date.now() - 86400000).toISOString();
    const tester = computeTrialExpired(
      school({ is_tester: true, subscription_status: "trial", trial_ends_at: pastDate }),
    );
