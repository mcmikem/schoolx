...
describe("Validation - Student Input", () => {
  ...
  describe("validateStudentInput", () => {
    ...
    test("allows partial updates without required field errors", () => {
      expect(validateStudentInput({ student_number: "SM001", opening_balance: 5000 }, { partial: true })).toEqual([]);
    });
    ...
    test("rejects invalid phone numbers", () => {
      ...
      ).toEqual(["Parent phone must be a valid phone number", "Alternative parent phone must be a valid phone number"]);
    });
    ...
    test("rejects duplicate parent phone numbers", () => {
      ...
      ).toEqual(["Alternative parent phone must be different from the primary parent phone"]);
    });
  });
});
