...
describe("buildAuthLoginAttempts", () => {
  it("produces normalized email attempt as primary format", () => {
    ...
    expect(attempts).toEqual(expect.arrayContaining([{ type: "email", value: "256700000000@omuto.org" }]));
  });

  it("includes phone fallback", () => {
    const attempts = buildAuthLoginAttempts("0700 000 000");

    expect(attempts).toEqual(expect.arrayContaining([{ type: "phone", value: "+256700000000" }]));
  });
