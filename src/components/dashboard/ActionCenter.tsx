  ...
  return (
    <div className="mb-8">
      <div className="flex items-center gap-2 mb-4">
        <MaterialIcon icon="bolt" style={{ color: "var(--amber)", fontSize: 20 }} />
        <h3 className="text-sm font-bold text-[var(--t1)] font-heading">Action Center</h3>
        ...
        {activeItems.map((item) => (
          ...
          >
            <div className={`w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 ${
              item.priority === "high" ? "bg-white/20" : "bg-[var(--amber-soft)] text-[var(--amber)]"
            }`}>
              <span className="text-xl font-extrabold font-heading">{item.count}</span>
            </div>
            <div className="min-w-0">
              <div className={`text-sm font-bold truncate ${item.priority === "high" ? "text-white" : "text-[var(--t1)]"}`}>
                {item.label}
              </div>
              <div className={`text-[11px] font-medium truncate ${item.priority === "high" ? "text-white/80" : "text-[var(--t3)]"}`}>
                {item.description}
              </div>
            </div>
