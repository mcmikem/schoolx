...
const roleBasedRoutes: Record<string, keyof RolePermissions> = deepFreeze({
  ...
  "/dashboard/students/graduation": "students",
  "/dashboard/students/alumni": "students",
  "/dashboard/students/photos": "students",
...
const PAGE_TITLE_OVERRIDES: Record<string, string> = deepFreeze({
  ...
  "/dashboard/students/graduation": "Graduation",
  "/dashboard/students/alumni": "Alumni",
  "/dashboard/students/photos": "Batch Photos",
...
export function getPageTitle(pathname: string): string {
  ...
  return (
    PAGE_TITLE_OVERRIDES[basePathname] ??
    pathname
      .replace("/dashboard/", "")
      .replace(/\/[0-9a-f-]{8,}(\/.+)?$/, "")
      .replace(/-/g, " ")
  ...
  const notifications = useMemo(() => {
    ...
    if (totalExpected > 0) {
      items.push({
        ...
        desc:
          outstanding > 0
            ? `${outstanding.toLocaleString()} UGX still outstanding across active students.`
            : "All expected fees are currently covered.",
