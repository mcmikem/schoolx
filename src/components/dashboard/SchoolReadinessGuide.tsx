...
export function SchoolReadinessGuide({ items, title = "School Readiness" }: SchoolReadinessGuideProps) {
  ...
  return (
    <div className="rounded-[20px] border border-[#f5deb3] bg-[#fffaf5] p-4 mb-6">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="ml-auto rounded-full bg-[#b45309] px-2 py-0.5 text-[10px] font-bold text-white">{missing} item{missing > 1 ? 's' : ''}</span>
      </div>
      <div className="space-y-2">
        {items.map((item) => (
          <div key={item.label} className={`rounded-xl border p-3 flex items-center gap-3 ${
            item.status === "ok"
              ? "border-[#d8efe7] bg-[#f3fbf8]"
              : "border-[#f5d0c5] bg-[#ffefe8]"
          }`}>
            ...
            {item.status !== "ok" && (
              <Link href={item.link} className="shrink-0 rounded-lg bg-[#17325f] px-3 py-1.5 text-[10px] font-bold text-white hover:opacity-90">
                Fix
              </Link>
            )}
          </div>
...
export function TeacherQuickGuide() {
  return (
    <div className="rounded-[20px] border border-[#d6e4e8] bg-[linear-gradient(150deg,#eff7f5_0%,#eaf2f6_44%,#f8fbff_100%)] p-4 mb-6">
      <div className="flex items-center gap-2 mb-3">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
        {[
          ...
          <Link key={action.step} href={action.href}
