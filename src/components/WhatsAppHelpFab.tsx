...
import { DEFAULT_WHATSAPP_ENV, generateSupportWhatsAppLink } from "@/lib/support-contact";

export default function WhatsAppHelpFab() {
  const { school, user } = useAuth();
  const pathname = usePathname();

  const schoolPhone = (school as { support_phone?: string } | null)?.support_phone || DEFAULT_WHATSAPP_ENV;

  const href = generateSupportWhatsAppLink({
    phone: schoolPhone,
