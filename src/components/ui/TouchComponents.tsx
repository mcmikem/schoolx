...
export function TouchButton({
  ...
  const variantClasses = {
    ...
    ghost: "bg-transparent text-[var(--on-surface)] hover:bg-[var(--surface-container)] focus:ring-[var(--primary)]",
  ...
  const sizeClasses = {
    sm: isTouch ? "min-h-[44px] px-3 py-2 text-sm gap-1.5" : "px-3 py-1.5 text-sm gap-1.5",
    md: isTouch ? "min-h-[44px] px-4 py-2.5 text-base gap-2" : "px-4 py-2 text-base gap-2",
    lg: isTouch ? "min-h-[52px] px-6 py-3 text-lg gap-2.5" : "px-6 py-2.5 text-lg gap-2.5",
  };
...
export function TouchInput({ label, error, hint, className = "", id, ...props }: TouchInputProps) {
  const isTouch = useIsTouchDevice();
    ...
    <div className="space-y-1.5">
      ...
      <input
        ...
        className={cn(
          ...
          error ? "border-red-500 focus:border-red-500" : "border-[var(--border)] focus:border-[var(--primary)]",
          className,
        )}
