...
function CollectionDonut({
  ...
  const partialPercent =
    stats.partialPaid > 0
      ? Math.round((stats.partialPaid / Math.max(stats.fullyPaid + stats.partialPaid + stats.notPaid, 1)) * 100)
      : 0;
  const unpaidPercent =
    stats.notPaid > 0
      ? Math.round((stats.notPaid / Math.max(stats.fullyPaid + stats.partialPaid + stats.notPaid, 1)) * 100)
      : 0;
    ...
    <div className="flex items-center gap-6">
      <div className="relative w-32 h-32 flex-shrink-0">
        <svg className="w-full h-full -rotate-90" viewBox="0 0 100 100">
          <circle cx="50" cy="50" r={radius} fill="none" stroke="var(--surface-container)" strokeWidth="12" />
          {stats.fullyPaid > 0 && (
            <circle
              cx="50"
              cy="50"
          ...
          {stats.partialPaid > 0 && (
            <circle
              ...
              style={{
                filter: hoveredSegment === "partial" ? "brightness(1.2)" : "none",
              }}
          ...
          {stats.notPaid > 0 && (
            <circle
              ...
              style={{
                filter: hoveredSegment === "unpaid" ? "brightness(1.2)" : "none",
              }}
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
          <span className="text-2xl font-bold text-on-surface">{paidPercent}%</span>
      ...
      <div className="flex flex-col gap-2 text-sm">
        ...
        >
          ...
          <span className="font-bold text-on-surface ml-auto">{stats.fullyPaid}</span>
        </button>
        <button
          onClick={() => onFilter?.("partial")}
        ...
        >
          ...
          <span className="font-bold text-on-surface ml-auto">{stats.partialPaid}</span>
        </button>
        <button
          onClick={() => onFilter?.("unpaid")}
        ...
        >
          ...
          <span className="font-bold text-on-surface ml-auto">{stats.notPaid}</span>
        </button>
      </div>
    </div>
  );
...
export default function FeeStats({ stats, paymentsCount, onFilter, activeFilter = "all" }: FeeStatsProps) {
  const totalStudents = stats.fullyPaid + stats.partialPaid + stats.notPaid;
  const collectionRate = totalStudents > 0 ? Math.round((stats.fullyPaid / totalStudents) * 100) : 0;
    ...
    <div className="mb-6">
      <div className="bg-surface-container-lowest rounded-xl p-6 mb-6">
        <div className="flex flex-col lg:flex-row items-center justify-between gap-6">
          <div>
            <h3 className="text-lg font-bold text-on-surface mb-1">Fee Collection Overview</h3>
            <p className="text-sm text-on-surface-variant mb-4">
              {formatCurrency(stats.totalPaid)} collected of {formatCurrency(stats.totalExpected)} expected
            </p>
            <CollectionDonut stats={stats} onFilter={onFilter} activeFilter={activeFilter} />
          </div>
          <div className="grid grid-cols-2 gap-4 w-full lg:w-auto">
            ...
            >
              ...
              <div className="text-xs text-green-700 font-medium">Collection Rate</div>
            </button>
            <button
              onClick={() => onFilter?.("unpaid")}
            ...
            >
              ...
              <div className="text-xs text-red-700 font-medium">Outstanding</div>
            </button>
            <div className="bg-blue-50 rounded-xl p-4 text-center">
              <div className="text-2xl font-bold text-blue-600">{stats.fullyPaid}</div>
              <div className="text-xs text-blue-700 font-medium">Fully Paid</div>
            </div>
            <div className="bg-amber-50 rounded-xl p-4 text-center">
              <div className="text-2xl font-bold text-amber-600">{stats.notPaid}</div>
      ...
      <div className="stat-grid">
        ...
        >
          <div className="flex justify-between items-start mb-4">
            <MaterialIcon icon="payments" className="text-secondary bg-secondary-container p-2 rounded-lg" />
        ...
        >
          <div className="flex justify-between items-start mb-4">
            <MaterialIcon icon="sync" className="text-tertiary bg-tertiary-fixed p-2 rounded-lg" />
            <span className="text-[10px] font-bold tracking-widest uppercase text-on-surface-variant opacity-60">
              Processing
            </span>
          </div>
