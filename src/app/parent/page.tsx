...
function ParentDashboard({ student, parentName, onLogout, onSwitchChild }: { student: any; parentName: string; onLogout: () => void; onSwitchChild: (child: any) => void }) {
  ...
  const averageGrade = grades.length > 0
    ? Math.round(grades.reduce((sum: number, item: any) => sum + Number(item.score || 0), 0) / grades.length)
    : 0
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
            ...
            {children.length > 1 && (
              <div style={{ marginTop: 12 }}>
                <select
                  value={student.id}
                  onChange={(e) => {
                    const next = children.find((child: any) => child.id === e.target.value)
                    if (next) onSwitchChild(next)
                  }}
                  style={{ width: '100%', padding: '10px 12px', borderRadius: 10, border: 'none', fontSize: 13 }}
                >
                  {children.map((child: any) => (
                    <option key={child.id} value={child.id}>
                      {child.first_name} {child.last_name} - {child.classes?.name}
                    </option>
                  ))}
                </select>
              </div>
      ...
      <div style={{ maxWidth: 600, margin: '0 auto', padding: 20 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 24 }}>
          ...
          <div style={{ background: 'var(--surface)', borderRadius: 12, padding: 16, textAlign: 'center' }}>
            <div style={{ fontSize: 28, fontWeight: 700, color: 'var(--navy)' }}>{averageGrade}%</div>
            <div style={{ fontSize: 12, color: 'var(--t3)' }}>Average Grade</div>
          </div>
          <div style={{ background: 'var(--surface)', borderRadius: 12, padding: 16, textAlign: 'center' }}>
            <div style={{ fontSize: 24, fontWeight: 700, color: feeStats.balance > 0 ? '#dc2626' : 'var(--green)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginBottom: 24 }}>
          <a href="#fees" style={{ background: 'var(--surface)', borderRadius: 12, padding: 16, textAlign: 'center', textDecoration: 'none', color: 'inherit' }}>
            <MaterialIcon icon="receipt_long" style={{ fontSize: 24, color: 'var(--navy)' }} />
            <div style={{ fontSize: 11, marginTop: 4, color: 'var(--t2)' }}>Fees</div>
          </a>
          <a href="#attendance" style={{ background: 'var(--surface)', borderRadius: 12, padding: 16, textAlign: 'center', textDecoration: 'none', color: 'inherit' }}>
            <MaterialIcon icon="how_to_reg" style={{ fontSize: 24, color: 'var(--navy)' }} />
            <div style={{ fontSize: 11, marginTop: 4, color: 'var(--t2)' }}>Attendance</div>
          </a>
          <a href="#grades" style={{ background: 'var(--surface)', borderRadius: 12, padding: 16, textAlign: 'center', textDecoration: 'none', color: 'inherit' }}>
            <MaterialIcon icon="menu_book" style={{ fontSize: 24, color: 'var(--navy)' }} />
            <div style={{ fontSize: 11, marginTop: 4, color: 'var(--t2)' }}>Grades</div>
          </a>
        </div>

        {/* Recent Attendance */}
        <div id="attendance" style={{ background: 'var(--surface)', borderRadius: 12, padding: 16, marginBottom: 16 }}>
        ...
        <div id="grades" style={{ background: 'var(--surface)', borderRadius: 12, padding: 16, marginBottom: 16 }}>
          <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 12, color: 'var(--t1)' }}>Recent Grades</div>
          {grades.length === 0 ? (
            <div style={{ textAlign: 'center', padding: 20, color: 'var(--t3)' }}>No grade records</div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {grades.slice(0, 5).map((grade: any) => (
                <div key={grade.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: '1px solid var(--border)' }}>
                  <span style={{ fontSize: 13, color: 'var(--t2)' }}>{grade.subjects?.name || 'Subject'}</span>
                  <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--navy)' }}>{Math.round(Number(grade.score || 0))}%</span>
                </div>
              ))}
            </div>
          )}
        ...
        {/* Recent Payments */}
        <div id="fees" style={{ background: 'var(--surface)', borderRadius: 12, padding: 16, marginBottom: 16 }}>
        ...
        <div style={{ background: 'var(--surface)', borderRadius: 12, padding: 16 }}>
          <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 12, color: 'var(--t1)' }}>Fee Adjustments</div>
          {adjustments.length === 0 ? (
            <div style={{ textAlign: 'center', padding: 20, color: 'var(--t3)' }}>No fee adjustments</div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {adjustments.slice(0, 5).map((adj: any) => (
                <div key={adj.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: '1px solid var(--border)' }}>
                  <div>
                    <div style={{ fontSize: 13, color: 'var(--t2)', textTransform: 'capitalize' }}>{String(adj.adjustment_type).replace('_', ' ')}</div>
                    <div style={{ fontSize: 11, color: 'var(--t3)' }}>{adj.notes || 'No notes'}</div>
                  </div>
                  <span style={{ fontSize: 13, fontWeight: 600, color: adj.adjustment_type === 'penalty' ? '#dc2626' : 'var(--navy)' }}>
