...
export default function ParentAcademicsPage() {
  ...
  const filtered = selectedTerm === "all" ? grades : grades.filter((grade) => grade.term === selectedTerm);

  const avgScore =
    filtered.length > 0
      ? Math.round(
          filtered.reduce((sum, grade) => sum + (grade.score / (grade.max_score || 100)) * 100, 0) / filtered.length,
        )
      : 0;

  const reportCards = useMemo(() => buildReportCardSummaries(grades), [grades]);
            ...
            <Card key={reportCard.term}>
              <CardBody className="space-y-4">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <p className="text-lg font-black text-[var(--on-surface)]">{reportCard.term} Report Card</p>
                    <p className="text-sm text-[var(--on-surface-variant)]">
                      {reportCard.subjectCount} subjects · strongest in {reportCard.strongestSubject}
                    </p>
                  </div>
                  <span
                ...
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <div className="rounded-[18px] bg-[var(--surface-container-low)] p-3 border border-[var(--border)]">
                    ...
                    <p className="text-sm font-bold text-[var(--on-surface)]">{reportCard.strongestSubject}</p>
                  </div>
                </div>
                ...
                <div className="rounded-[20px] border border-[var(--border)] p-4 bg-[var(--surface-container-low)]">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <p className="text-sm font-bold text-[var(--on-surface)]">Teacher remark</p>
                  </div>
                  <p className="text-sm text-[var(--on-surface-variant)] italic">
                    {reportCard.teacherComment || "Teacher remarks will appear here once they are recorded."}
                  </p>
                </div>
              </CardBody>
            </Card>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start justify-between mb-2">
                        <div>
                          <p className="font-bold text-[var(--on-surface)]">{grade.subject_name}</p>
                        ...
                        <div className="text-right">
                          ...
                          {grade.grade && (
                            <span className="text-xs font-black text-[var(--on-surface-variant)]">{grade.grade}</span>
                          )}
                        </div>
                      </div>
