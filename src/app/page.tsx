...
const securityDetails = [
  { icon: 'fingerprint', title: 'Who can see what', desc: 'Every role has strict boundaries. A teacher sees only their classes. A parent sees only their child. A bursar sees only finances. Even if someone shares a login, the system limits what they can access.' },
  { icon: 'admin_panel_settings', title: 'Data ownership', desc: 'Your school data belongs to you — not to us. You can export everything at any time. If you leave, we delete your data within 30 days. We never use school data for training, advertising, or any purpose other than running your system.' },
  { icon: 'security', title: 'Infrastructure security', desc: 'Hosted on Supabase (PostgreSQL) with row-level security policies. All connections use TLS 1.3. Database backups run continuously. Your data is encrypted at rest with AES-256.' },
  { icon: 'gavel', title: 'Uganda compliance', desc: 'Built to align with Uganda\'s Data Protection and Privacy Act, 2019. Student records, parent contacts, and financial data are handled according to national requirements for personal information.' },
  { icon: 'devices', title: 'Device safety', desc: 'Sessions expire after inactivity. Each login requires authentication. If a teacher loses their phone, you can revoke their access instantly from the dashboard without affecting anyone else.' },
  { icon: 'history', title: 'Audit trail', desc: 'Every fee entry, grade change, and attendance correction is logged. You can see who changed what and when. This protects against accidental edits and makes accountability clear.' },
]
...
const tabContent: Record<number, { stats: [string, string, string][]; bars?: { name: string; value: number; color: string }[]; actions?: { title: string; note: string; icon: string }[]; students?: { name: string; class: string; balance: string }[] }> = {
  0: {
    stats: [['Fees this term', 'UGX 18.4M', 'cash, bank, and mobile money'], ['Staff on duty', '43', 'present and late captured'], ['Low attendance', '3', 'follow-up before assembly']],
    bars: [{ name: 'S.1', value: 78, color: 'bg-[#17325F]' }, { name: 'S.2', value: 84, color: 'bg-[#2E9448]' }, { name: 'S.3', value: 71, color: 'bg-[#B86B0C]' }, { name: 'S.4', value: 88, color: 'bg-[#17325F]' }],
    actions: [{ title: 'Approve expense request', note: 'Lab practical supplies', icon: 'payments' }, { title: 'Review leave request', note: '2 staff awaiting action', icon: 'event_busy' }, { title: 'Send fee reminder', note: 'S.2 Blue parents with balances', icon: 'sms' }, { title: 'Print report cards', note: 'S.4 candidate review', icon: 'description' }],
  },
  1: {
    stats: [['Total students', '847', 'active across 24 classes'], ['New this term', '62', 'admissions processed'], ['Transfers', '5', 'pending approval']],
    students: [{ name: 'Nakamya Sarah', class: 'S.3 Blue', balance: 'UGX 185,000' }, { name: 'Ochen David', class: 'S.2 Red', balance: 'UGX 0' }, { name: 'Achieng Grace', class: 'S.4 Green', balance: 'UGX 320,000' }, { name: 'Mugisha Brian', class: 'S.1 Blue', balance: 'UGX 0' }],
  },
  2: {
    stats: [['Present today', '782', 'out of 847 students'], ['Absent', '65', '7.7% absence rate'], ['Late', '12', 'arrived after 8:00 AM']],
    bars: [{ name: 'P.7', value: 92, color: 'bg-[#2E9448]' }, { name: 'S.1', value: 85, color: 'bg-[#17325F]' }, { name: 'S.2', value: 78, color: 'bg-[#B86B0C]' }, { name: 'S.4', value: 95, color: 'bg-[#2E9448]' }],
  },
  3: {
    stats: [['Exams this term', '4', 'CA, BOT, Mid Term, EOT'], ['Grades entered', '89%', '21 of 24 classes'], ['Report cards', 'Ready', 'S.4 candidates']],
    bars: [{ name: 'S.1', value: 78, color: 'bg-[#17325F]' }, { name: 'S.2', value: 84, color: 'bg-[#2E9448]' }, { name: 'S.3', value: 71, color: 'bg-[#B86B0C]' }, { name: 'S.4', value: 88, color: 'bg-[#17325F]' }],
  },
  4: {
    stats: [['Collected', 'UGX 18.4M', 'this term'], ['Outstanding', 'UGX 6.2M', 'across 127 students'], ['Today', 'UGX 840K', '3 payments']],
    actions: [{ title: 'Record payment', note: 'Walk-in or mobile money', icon: 'add_card' }, { title: 'Send fee reminder', note: '127 parents with balances', icon: 'sms' }, { title: 'Generate invoice', note: 'S.2 Blue class', icon: 'description' }, { title: 'View cashbook', note: 'All transactions', icon: 'book' }],
  },
  5: {
    stats: [['SMS sent today', '426', 'bulk to S.4 parents'], ['Delivery rate', '97%', '413 delivered'], ['Templates', '12', 'saved']],
    actions: [{ title: 'Send bulk SMS', note: 'All parents, fee reminder', icon: 'sms' }, { title: 'New template', note: 'Save reusable message', icon: 'note_add' }, { title: 'View delivery', note: 'Track sent messages', icon: 'track_changes' }],
  },
}
...
function DesktopMockup() {
  ...
  const handleKeyDown = (e: React.KeyboardEvent, index: number) => {
    if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); setActiveTab(index) }
  }
          ...
          <div className="space-y-4 p-4 sm:p-5">
            <div className="grid gap-3 md:grid-cols-3">
              {content.stats.map(([label, value, note], i) => {
                ...
                return (
                  <div key={label} ref={counter.ref} className="story-card rounded-[24px] bg-white p-4 shadow-sm ring-1 ring-slate-200 hover:shadow-md transition-shadow cursor-default">
                ...
                <div className="mb-4 flex items-center justify-between">
                  <div>
                    <p className="text-sm font-semibold text-slate-900">
                      {activeTab === 2 ? 'Attendance by class' : activeTab === 3 ? 'Candidate and class performance' : 'Performance overview'}
                    </p>
                    ...
                    <div key={bar.name}>
                      ...
                      <div className="h-2.5 rounded-full bg-slate-100 overflow-hidden">
                        <div
                          ...
                          style={{ width: mounted ? `${bar.value}%` : '0%', transitionDelay: `${i * 200 + 300}ms` }}
                        />
                      </div>
                    </div>
                  ))}
                ...
                <div className="space-y-3">
                  {content.students.map((s) => (
                    <div key={s.name} className="flex items-center justify-between py-2 border-b border-slate-100 last:border-0">
                      ...
                      <span className={`text-xs font-semibold px-2 py-1 rounded-full ${s.balance === 'UGX 0' ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'}`}>
                        {s.balance}
                      </span>
                    </div>
                  ))}
...
const smsMessages = [
  { from: 'SchoolX', text: 'Dear parent, your child was absent today and current Term II fee balance is UGX 185,000. Please contact the office if you need a statement.', type: 'incoming' },
  { from: 'System', text: 'Bulk SMS delivered — S.4 candidates, 8:14 AM. 426 recipients, 147/160 characters.', type: 'outgoing' },
]
          ...
          <div className="flex justify-center gap-2 pt-1">
            {smsMessages.map((_, i) => (
              <button
                ...
                onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); setActiveMsg(i) } }}
                tabIndex={0}
  ...
  return (
    <div className="border border-slate-200 rounded-2xl overflow-hidden bg-white hover:shadow-sm transition-shadow">
      ...
      >
        ...
        <span className={`material-symbols-outlined text-[#17325F] transition-transform duration-300 flex-shrink-0 ${open ? 'rotate-180' : ''}`}>
          expand_more
        </span>
      </button>
  ...
  return (
    <main className="min-h-screen bg-[#f4f7fb] text-slate-900">
        ...
        <div className="relative mx-auto max-w-7xl px-4 pb-16 pt-5 sm:px-6 lg:px-8 lg:pb-24">
          ...
          <nav className="flex items-center justify-between rounded-full border border-white/70 bg-white/80 px-4 py-3 shadow-[0_8px_30px_rgba(15,23,42,0.06)] backdrop-blur sm:px-6">
            ...
            <div className="hidden items-center gap-8 text-sm font-medium text-slate-600 md:flex">
              <button onClick={() => smoothScroll('#features')} className="transition hover:text-slate-950 cursor-pointer">Features</button>
              <button onClick={() => smoothScroll('#story')} className="transition hover:text-slate-950 cursor-pointer">Story</button>
              <button onClick={() => smoothScroll('#security')} className="transition hover:text-slate-950 cursor-pointer">Security</button>
              <button onClick={() => smoothScroll('#pricing')} className="transition hover:text-slate-950 cursor-pointer">Pricing</button>
            </div>
      ...
      <section className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
        <FadeIn>
          <div className="text-center mb-10">
            ...
            <h2 className="mt-3 font-['Sora'] text-2xl font-semibold text-slate-950 sm:text-3xl">From signup to running your school</h2>
          </div>
          <div className="grid gap-6 sm:grid-cols-3">
            {[
              { step: '1', icon: 'how_to_reg', title: 'Register your school', desc: 'Create your account in under 2 minutes. No credit card needed.' },
              { step: '2', icon: 'group_add', title: 'Add your students', desc: 'Upload a CSV or add students manually. Create classes and subjects.' },
              { step: '3', icon: 'rocket_launch', title: 'Start using it', desc: 'Mark attendance, record fees, send SMS — your school runs from one place.' },
            ].map((item, i) => (
      ...
      <section className="mx-auto max-w-3xl px-4 py-18 sm:px-6 lg:px-8 lg:py-24">
        <FadeIn>
          <div className="text-center mb-10">
            <p className="text-sm font-semibold uppercase tracking-[0.24em] text-[#17325F]">Frequently asked questions</p>
            <h2 className="mt-3 font-['Sora'] text-2xl font-semibold text-slate-950 sm:text-3xl">Questions schools ask before signing up</h2>
          </div>
          <div className="space-y-3">
            {faqItems.map((item, i) => (
          ...
          <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div>
              ...
              <ul className="mt-4 space-y-3">
                <li><button onClick={() => smoothScroll('#features')} className="text-sm text-slate-600 hover:text-slate-900 transition cursor-pointer">Features</button></li>
                <li><button onClick={() => smoothScroll('#pricing')} className="text-sm text-slate-600 hover:text-slate-900 transition cursor-pointer">Pricing</button></li>
                <li><button onClick={() => smoothScroll('#security')} className="text-sm text-slate-600 hover:text-slate-900 transition cursor-pointer">Security</button></li>
              </ul>
            </div>
            <div>
