...
const securityFeatures = [
  {
    ...
    desc: 'All student records, grades, and financial data are encrypted in transit and at rest using industry-standard TLS and AES-256.',
  },
  {
    icon: 'shield',
    title: 'Role-Based Access',
    desc: 'Head teachers, bursars, teachers, and parents each see only what they need. No one can access data outside their permission level.',
  },
  {
    icon: 'cloud_done',
    title: 'Secure Cloud Hosting',
    desc: 'Hosted on Supabase with PostgreSQL — a battle-tested database trusted by thousands of organisations worldwide.',
  },
  {
    icon: 'backup',
    title: 'Automatic Backups',
    desc: 'Your school data is backed up continuously. Nothing is lost if a device fails or someone makes a mistake.',
  },
  {
    icon: 'privacy_tip',
    title: 'Data Privacy',
    desc: 'Student data belongs to the school. We never sell, share, or use your data for advertising. Period.',
  },
  {
    icon: 'verified_user',
    title: 'Compliance Ready',
    desc: 'Built to support Uganda\'s Data Protection and Privacy Act requirements for handling personal information.',
  },
]

const trustSignals = [
  { icon: 'verified', text: 'Built by Omuto Foundation — a registered Ugandan organisation' },
  { icon: 'cloud', text: 'Hosted on Supabase (PostgreSQL) — trusted by 100,000+ organisations' },
  { icon: 'https', text: 'All connections secured with SSL/TLS encryption' },
  { icon: 'support_agent', text: 'Local support team available via WhatsApp, email, and phone' },
]
        ...
        <div className="relative mx-auto max-w-7xl px-4 pb-16 pt-5 sm:px-6 lg:px-8 lg:pb-24">
          ...
          <nav className="flex items-center justify-between rounded-full border border-white/70 bg-white/80 px-4 py-3 shadow-[0_8px_30px_rgba(15,23,42,0.06)] backdrop-blur sm:px-6">
            ...
            <div className="hidden items-center gap-8 text-sm font-medium text-slate-600 md:flex">
              ...
              <a href="#osx" className="transition hover:text-slate-950">OSX</a>
      ...
      <section id="security" className="mx-auto max-w-7xl px-4 py-18 sm:px-6 lg:px-8 lg:py-24">
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {securityFeatures.map((feature) => (
            <div key={feature.title} className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm hover:shadow-md transition-shadow">
        ...
        {/* Trust signals */}
        <div className="mt-14 rounded-[34px] border border-slate-200 bg-[#f8fbff] p-6 sm:p-8">
          <div className="flex items-center gap-3 mb-6">
            <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-[#17325F] text-white">
              <MaterialIcon icon="verified" className="text-[22px]" />
            </div>
            <div>
              <h3 className="text-lg font-semibold text-slate-900">Trusted Infrastructure</h3>
              <p className="text-sm text-slate-500">Built on platforms trusted by organisations worldwide</p>
            </div>
          </div>
          <div className="grid gap-4 sm:grid-cols-2">
            {trustSignals.map((signal) => (
              <div key={signal.text} className="flex items-start gap-3 rounded-[20px] bg-white px-4 py-3 ring-1 ring-slate-200">
                <div className="mt-0.5 flex h-8 w-8 items-center justify-center rounded-xl bg-[#eaf4ed] text-[#2E9448] flex-shrink-0">
                  <MaterialIcon icon={signal.icon} className="text-[18px]" />
                </div>
                <p className="text-sm leading-6 text-slate-700">{signal.text}</p>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div>
              ...
              <ul className="mt-4 space-y-3">
                ...
                <li><a href="#osx" className="text-sm text-slate-600 hover:text-slate-900 transition">OSX Integration</a></li>
              </ul>
            </div>
            <div>
