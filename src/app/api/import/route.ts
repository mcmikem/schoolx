  ...
  try {
    ...
    for (const student of students as StudentRow[]) {
      try {
        ...
        // Validate gender
        const gender = student.gender?.toUpperCase() === 'M' || student.gender?.toUpperCase() === 'MALE'
          ? 'M'
        ...
        results.errors.push(`${student.first_name} ${student.last_name}: ${errorMessage}`)
        results.failed++
      }
    }
...
export const POST = withSecurity(handlePost, { rateLimit: { limit: 10, windowMs: 60000 } })
