...
export async function POST(request: NextRequest) {
  try {
        ...
        sql: `
          ...
          BEGIN
            ...
            IF v_school_id IS NULL THEN
              RAISE EXCEPTION 'Cannot top up wallet: student not found or not associated with a school';
            END IF;
    ...
    const migrations = [
      ...
      `CREATE POLICY parent_messages_parent_select ON parent_messages FOR SELECT USING (parent_id IN (SELECT id FROM users WHERE auth_id = auth.uid()));`,
      `DROP POLICY IF EXISTS parent_messages_parent_insert ON parent_messages;`,
      `CREATE POLICY parent_messages_parent_insert ON parent_messages FOR INSERT WITH CHECK (parent_id IN (SELECT id FROM users WHERE auth_id = auth.uid()));`,
    ...
    if (shouldSeedDemo) {
      ...
      results["demo_seeding"] = "Skipped (not enabled or in production)";
    }
