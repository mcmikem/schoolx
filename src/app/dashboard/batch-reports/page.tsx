...
export default function BatchReportsPage() {
  ...
  const [step, setStep] = useState<
    "select" | "generating" | "preview" | "done"
  >("select");
    ...
    try {
      ...
      for (let i = 0; i < students.length; i++) {
        ...
        for (const subject of subjects) {
          ...
          const ca1 =
            grades?.find((g) => g.assessment_type === "ca1")?.score || 0;
          const ca2 =
            grades?.find((g) => g.assessment_type === "ca2")?.score || 0;
          const ca3 =
            grades?.find((g) => g.assessment_type === "ca3")?.score || 0;
          const ca4 =
            grades?.find((g) => g.assessment_type === "ca4")?.score || 0;
          const project =
            grades?.find((g) => g.assessment_type === "project")?.score || 0;
          const exam =
            grades?.find((g) => g.assessment_type === "exam")?.score || 0;

          const { totalCA, finalScore, grade } = computeSubjectTotal(
            ca1,
            ca2,
            ca3,
            ca4,
  ...
  const handleDownloadAll = async () => {
    ...
    for (const report of reports) {
      console.log(
        `Generating PDF for ${report.first_name} ${report.last_name}`,
      );
    }
  };
          ...
          <CardBody>
            <div className="text-center py-12">
              ...
              <p className="text-sm text-gray-500 mb-6">
                Computing grades, positions, and comments
              </p>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                ...
                <div className="text-center">
                  <div className="text-2xl font-bold text-green-600">
                    {reports.filter((r) => r.overall.average >= 70).length}
                  </div>
                  ...
                  <div className="text-2xl font-bold text-blue-600">
                    {
                      reports.filter(
                        (r) =>
                          r.overall.average >= 50 && r.overall.average < 70,
                      ).length
                    }
                  </div>
                ...
                <div className="text-center">
                  <div className="text-2xl font-bold text-red-600">
                    {reports.filter((r) => r.overall.average < 50).length}
                  </div>
                    ...
                    <div className="flex items-center gap-6">
                      <div className="text-right">
                        <div
                          ...
                          style={{
                            color:
                              report.overall.average >= 70
                                ? "#16a34a"
                                : report.overall.average >= 50
                                  ? "#2563eb"
                                  : "#dc2626",
                          }}
                        >
                      ...
                      <div className="text-right hidden sm:block">
                        ...
                        <div className="text-xs text-gray-400">
                          {
                            report.subjects.filter((s) => s.finalScore >= 70)
                              .length
                          }{" "}
                          distinction
                        </div>
