...
export default function CommunicationHubPage() {
  ...
  const [messages, setMessages] = useState<
    Array<{ id: string; message: string; recipient_type: string; status: string; created_at: string }>
  >([]);
  ...
  const noticeValidationError =
    !newNotice.title.trim() || !newNotice.content.trim() ? "Add both title and content to post this notice." : "";
  ...
  const deliverMessage = async (phones: string[], body: string) => {
    ...
    if (preferWhatsApp) {
      const results = await Promise.all(phones.map((recipientPhone) => sendWhatsAppTextMessage(recipientPhone, body)));
    ...
    const smsStatus = resultData.status;
    const sentCount =
      typeof resultData.totalSent === "number" ? resultData.totalSent : smsStatus === "sent" ? phones.length : 0;
    const failedCount =
      typeof resultData.totalFailed === "number" ? resultData.totalFailed : Math.max(0, phones.length - sentCount);
    return {
      success: sentCount > 0,
      totalSent: sentCount,
    ...
    async function loadAbsencePreview() {
      ...
      try {
        ...
        const alerts = detectConsecutiveAbsenceAlerts({
          ...
          trigger: { threshold_days: threshold, is_active: absenceTrigger?.is_active ?? true },
        });
    ...
    try {
      ...
      if (isDemo) {
        if (messageType === "individual") {
          ...
          phones = DEMO_STUDENTS.filter((s) => s.class_id === selectedClass && s.status === "active")
            .map((s) => s.parent_phone)
            .filter(Boolean);
        } else {
          phones = DEMO_STUDENTS.filter((s) => s.school_id === school?.id && s.status === "active")
            .map((s) => s.parent_phone)
            .filter(Boolean);
        }
      ...
      if (delivery.success) {
        ...
        toast.success(
          `Sent via ${delivery.via.toUpperCase()} to ${phones.length} recipient${phones.length > 1 ? "s" : ""}`,
        );
        setMessage("");
        setPhone("");
        fetchMessagesData();
      } else {
        toast.error(delivery.result?.message || "Failed to send");
      }
    } catch (err: unknown) {
  ...
  const openCreateRule = () => {
    ...
    setRuleForm({ name: "", event_type: "student_absent", threshold_days: 3, is_active: true });
  ...
  const deleteTemplate = async (id: string) => {
    setPendingAction(() => async () => {
      try {
        await deleteRecord(() => supabase.from("sms_templates").delete().eq("id", id), {
          timeoutMs: 10000,
          timeoutMessage: "Template delete timed out",
        });
  ...
  const createDefaultTemplates = async () => {
    try {
      ...
      const templatesToCreate = DEFAULT_TEMPLATES.filter((t) => !existingNames.has(t.name)).map((t) => ({
        school_id: school?.id,
        ...t,
        is_active: true,
        created_by: user?.id,
      ...
      await createRecord(() => supabase.from("sms_templates").insert(templatesToCreate), {
        timeoutMs: 12000,
        timeoutMessage: "Default template creation timed out",
      });
  ...
  const handleBulkSend = async () => {
    ...
    try {
      ...
      if (delivery.success) {
        ...
        toast.success(`${delivery.via.toUpperCase()} sent to ${phones.length} parent${phones.length > 1 ? "s" : ""}`);
        setBulkMessage("");
        setSelectedTemplateId("");
        setShowConfirm(false);
      } else {
        toast.error(delivery.result?.message || "Failed to send message");
      }
    } catch (err: unknown) {
  ...
  const deleteNotice = async (id: string) => {
    setPendingAction(() => async () => {
      ...
      try {
        await deleteRecord(() => supabase.from("notices").delete().eq("id", id), {
          timeoutMs: 10000,
          timeoutMessage: "Notice delete timed out",
        });
        setNotices(notices.filter((n) => n.id !== id));
        toast.success("Notice deleted");
      } catch (err: unknown) {
  ...
  const acknowledgeNotice = async (noticeId: string) => {
    ...
    if (isDemo) {
      setNotices((prev) =>
        prev.map((n) =>
          n.id === noticeId
            ? { ...n, acknowledged_by: Array.from(new Set([...(n.acknowledged_by || []), user.id])) }
            : n,
        ),
      );
    ...
    try {
      await upsertRecord(
        () =>
          supabase.from("notice_acknowledgments").upsert(
            {
              notice_id: noticeId,
              user_id: user.id,
  ...
  return (
    <PageErrorBoundary>
      <div className="p-4 sm:p-6 lg:p-8">
        ...
        <p className="-mt-3 mb-6 text-sm text-[var(--t3)]">{communicationTabHints[activeTab]}</p>
              ...
              >
                ...
                <form onSubmit={handleNoticeSubmit} className="p-6 space-y-4">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {newNotice.image_url && (
                        <span className="text-sm text-green-600 flex items-center gap-1">
                          <MaterialIcon className="text-sm">check_circle</MaterialIcon>Image attached
                        </span>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex gap-3 pt-4">
                    ...
                    <Button
                      type="submit"
                      disabled={sendingSMS || Boolean(noticeValidationError)}
                      loading={sendingSMS}
                      className="flex-1"
                    >
                      Post Notice
