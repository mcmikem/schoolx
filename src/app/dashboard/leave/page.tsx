...
export default function LeavePage() {
  ...
  const getStatusBadge = (status: string) => {
    const styles: Record<string, { bg: string; text: string; label: string }> = {
      pending: { bg: 'bg-[var(--amber-soft)]', text: 'text-[var(--amber)]', label: 'Pending' },
      dos_approved: { bg: 'bg-[var(--navy-soft)]', text: 'text-[var(--navy)]', label: 'DOS Approved' },
      approved: { bg: 'bg-[var(--green-soft)]', text: 'text-[var(--green)]', label: 'HM Approved' },
      rejected: { bg: 'bg-[var(--red-soft)]', text: 'text-[var(--red)]', label: 'Rejected' },
    }
        ...
        <Card>
          <CardBody>
            <EmptyState
              ...
              action={{ label: 'Request Leave', onClick: () => setShowModal(true) }}
            />
          </CardBody>
        </Card>
      ) : (
              ...
              <CardBody>
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    {req.reason && (
                      <div className="text-sm text-[var(--t3)] mt-1">{req.reason}</div>
                    )}
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4" onClick={() => setShowModal(false)}>
          ...
          <Card className="w-full max-w-md">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                <label className="text-sm font-medium text-[var(--on-surface)] mb-2 block">Substitute Suggestion (Optional)</label>
                <input
                  type="text"
