...
const AUTOMATIONS = [
  {
    ...
    description: 'Auto-send SMS to parents when fees are overdue (7, 14, 30 days)',
    icon: 'payments',
    color: 'amber',
  },
  {
  ...
  },
  {
    ...
    description: 'Alert parents when term report cards are ready for collection',
    icon: 'school',
    color: 'blue',
  },
]
      ...
      {activeTab === 'automations' ? (
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-4">
                        ...
                        >
                          <div className={`absolute top-0.5 w-6 h-6 bg-white rounded-full shadow transition-transform ${isEnabled ? 'left-5' : 'left-0.5'}`} />
                        </button>
                      </div>
        ...
        <div>
          ...
          <div className="flex gap-2 mb-4">
            {['all', 'fee_overdue', 'absentee_alert', 'payment_confirmation', 'report_card_ready'].map(f => (
              <button
                key={f}
          ...
          {loading ? (
            <div className="space-y-3">{[1,2,3,4].map(i => <div key={i} className="card"><div className="skeleton h-4 w-3/4 mb-2"></div><div className="skeleton h-3 w-1/2"></div></div>)}</div>
                  ...
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                        ...
                        log.status === 'demo' ? 'bg-blue-100 text-blue-700' :
                        'bg-red-100 text-red-700'
                      }`}>{log.status}</span>
                      <span className="text-sm font-medium text-gray-900">{log.studentName || log.studentId}</span>
                    </div>
                    <span className="text-xs text-gray-400">{new Date(log.timestamp).toLocaleString()}</span>
                  </div>
                  <p className="text-sm text-gray-600 line-clamp-2">{log.message}</p>
                  <div className="text-xs text-gray-400 mt-1">To: {log.parentPhone}</div>
                </div>
              ))}
            </div>
          )}
