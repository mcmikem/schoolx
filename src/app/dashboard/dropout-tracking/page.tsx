...
function buildDemoAtRiskStudents(): AtRiskStudent[] {
  ...
  return demoEntries.reduce<AtRiskStudent[]>((accumulator, entry) => {
    ...
    accumulator.push({
      ...
      class: DEMO_CLASSES.find((classItem) => classItem.id === student.class_id)?.name || "-",
...
export default function DropoutTrackingPage() {
  ...
  const fetchAtRiskStudents = useCallback(async () => {
    ...
    for (const entry of Array.from(studentMap.entries())) {
      const sid = entry[0];
      const data = entry[1];
      const sortedDates = data.dates.sort((a: string, b: string) => b.localeCompare(a));
  ...
  return (
    <PageErrorBoundary>
      <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500">
        <PageHeader title="Dropout Tracking" subtitle="Students with high consecutive absence streaks" />
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-500">
                          {s.class} · {s.consecutiveAbsence} consecutive days absent
                        </p>
                      </div>
                    </div>
            ...
            <div className="bg-[var(--surface)] rounded-3xl w-full max-w-md max-h-[calc(100vh-1.5rem)] sm:max-h-[calc(100vh-2rem)] overflow-y-auto my-auto shadow-2xl p-8 space-y-5">
              ...
              <div className="p-3 bg-amber-50 rounded-2xl border border-amber-200">
                ...
                <p className="text-xs text-amber-600">
                  {selectedStudent.class} · {selectedStudent.consecutiveAbsence} days absent
                </p>
              </div>
              <div>
                <label
        ...
        {showDropoutModal && selectedStudent && (
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-start sm:items-center justify-center overflow-y-auto p-3 sm:p-4">
            <div className="bg-[var(--surface)] rounded-3xl w-full max-w-md max-h-[calc(100vh-1.5rem)] sm:max-h-[calc(100vh-2rem)] overflow-y-auto my-auto shadow-2xl p-8 space-y-5">
              <div className="flex justify-between items-center">
                <h2 className="text-xl font-black text-[var(--on-surface)]">Mark as Dropout</h2>
                <button
