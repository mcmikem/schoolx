    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500">
        ...
        {confirmOpen && (
          <div className="fixed inset-0 z-50 flex items-start sm:items-center justify-center overflow-y-auto bg-black/50 p-3 sm:p-4">
            <div className="bg-white rounded-2xl p-6 w-full max-w-sm max-h-[calc(100vh-1.5rem)] sm:max-h-[calc(100vh-2rem)] overflow-y-auto my-auto shadow-2xl">
              ...
              <p className="text-sm text-slate-500 mb-1">{staff.length} staff members &bull; Gross: <span className="font-semibold text-slate-700">UGX {totals.gross.toLocaleString()}</span> &bull; Net: <span className="font-semibold text-slate-700">UGX {totals.net.toLocaleString()}</span></p>
              <p className="text-xs text-amber-600 bg-amber-50 rounded-xl p-3 mt-3">
                This will calculate PAYE tax and create payroll records for all staff. The records will appear in Payroll History.
              </p>
              <div className="flex gap-3 mt-5">
                ...
                <button onClick={handleRunPayroll} className="flex-1 py-2.5 rounded-xl bg-emerald-600 text-white text-sm font-semibold hover:bg-emerald-700 transition">Confirm & Run</button>
              </div>
            </div>
          </div>
        )}
              ...
              {progress.error ? (
                ...
                <div className="space-y-3">
                  ...
                  <div className="bg-slate-50 rounded-xl p-3 space-y-1 text-sm">
                    <p className="flex justify-between"><span className="text-slate-500">Gross total</span><span className="font-bold text-slate-800">UGX {progress.totalGross.toLocaleString()}</span></p>
                    <p className="flex justify-between"><span className="text-slate-500">Total tax (PAYE+NSSF)</span><span className="font-bold text-rose-600">UGX {progress.totalTax.toLocaleString()}</span></p>
                    <p className="flex justify-between"><span className="text-slate-500">Net total</span><span className="font-bold text-emerald-600">UGX {progress.totalNet.toLocaleString()}</span></p>
                  </div>
                </div>
              )}
              <button
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          { label: "Gross Payroll", value: `UGX ${(totals.gross / 1000000).toFixed(1)}M`, icon: "payments", color: "bg-blue-600", suffix: "" },
          { label: "Total PAYE Tax", value: `UGX ${totals.paye.toLocaleString()}`, icon: "gavel", color: "bg-red-500", suffix: "" },
          { label: "Total Net Pay", value: `UGX ${(totals.net / 1000000).toFixed(1)}M`, icon: "account_balance_wallet", color: "bg-emerald-600", suffix: "" },
        ].map((s) => (
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-slate-50/50">
                ...
                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-slate-400 text-right">Net Pay</th>
          ...
          {historyLoading ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                <thead>
                  <tr className="bg-slate-50/50">
                    <th className="px-4 py-3 text-[9px] font-black uppercase tracking-widest text-slate-400">Month</th>
                    <th className="px-4 py-3 text-[9px] font-black uppercase tracking-widest text-slate-400 text-right">Gross</th>
                    <th className="px-4 py-3 text-[9px] font-black uppercase tracking-widest text-slate-400 text-right">NSSF</th>
                    <th className="px-4 py-3 text-[9px] font-black uppercase tracking-widest text-slate-400 text-right">PAYE</th>
                    <th className="px-4 py-3 text-[9px] font-black uppercase tracking-widest text-slate-400 text-right">Net</th>
                ...
                <tbody className="divide-y divide-slate-50">
                  {history.map((r) => (
                    <tr key={r.id} className="hover:bg-slate-50/30 transition-colors">
                      ...
                      <td className="px-4 py-3 text-xs font-bold text-slate-800 text-right">{r.gross_pay.toLocaleString()}</td>
                      <td className="px-4 py-3 text-xs font-bold text-amber-600 text-right">{r.nssf_deduction.toLocaleString()}</td>
                      <td className="px-4 py-3 text-xs font-bold text-rose-600 text-right">{r.paye_tax.toLocaleString()}</td>
                      <td className="px-4 py-3 text-xs font-bold text-emerald-700 text-right">{r.net_pay.toLocaleString()}</td>
                      <td className="px-4 py-3">
                        <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full ${r.status === "paid" ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}`}>
                          {r.status}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-[10px] text-slate-400">{format(new Date(r.created_at), "dd MMM yyyy")}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
