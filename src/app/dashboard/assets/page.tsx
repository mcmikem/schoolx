...
export default function AssetsPage() {
  ...
  const fetchAssets = useCallback(async () => {
    if (!school?.id) return;
    setLoading(true);
    const { data, error } = await supabase.from("assets").select("*").eq("school_id", school.id).order("name");
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            ...
            },
            {
              ...
              value: assets.filter((a) => a.condition === "Requires Maintenance").length,
              icon: "build",
              color: "bg-amber-500",
            },
            {
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-[var(--border)]/60">
                  {filtered.map((a) => (
                    <tr key={a.id} className="transition-colors hover:bg-[var(--surface-container-low)]/70">
                      <td className="px-6 py-4 text-sm font-semibold text-[var(--on-surface)]">{a.name}</td>
                      <td className="px-6 py-4 text-sm text-[var(--on-surface-variant)]">{a.category}</td>
                      <td className="px-6 py-4 text-sm text-[var(--on-surface-variant)]">{a.location}</td>
            ...
            <div className="w-full max-w-lg max-h-[calc(100vh-1.5rem)] sm:max-h-[calc(100vh-2rem)] overflow-y-auto my-auto rounded-[34px] border border-white/70 bg-white shadow-[0_24px_60px_rgba(15,23,42,0.2)]">
              <div className="p-8 space-y-5">
                <div className="flex justify-between items-center">
                  <h2 className="text-2xl font-semibold tracking-[-0.03em] text-[var(--on-surface)]">Register Asset</h2>
                  <button
