...
export default function CoursesPage() {
  ...
  const fetchCourses = useCallback(async () => {
    ...
    const loadSubjects = async () => {
      ...
      setCourses(filterCategory ? mapped.filter((course) => course.category === filterCategory) : mapped);
    ...
    try {
      ...
      let query = supabase.from("courses").select("*").eq("school_id", school.id).order("name");
  ...
  const toggleActive = async (id: string, isActive: boolean) => {
    try {
      const { error } = await supabase.from("courses").update({ is_active: !isActive }).eq("id", id);
  ...
  const getCategoryStyle = (category: string) => {
    return COURSE_CATEGORIES.find((c) => c.value === category)?.color || "bg-gray-100 text-gray-700";
  };
      ...
      <>
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            <MaterialIcon icon="menu_book" className="text-5xl text-gray-300 mx-auto mb-4" />
            ...
            {courses.map((course) => (
              <div
                ...
                className={`bg-white rounded-2xl border p-4 ${
                  course.is_active ? "border-gray-200" : "border-gray-100 bg-gray-50"
                }`}
              >
                <div className="flex items-start justify-between mb-3">
                  ...
                  <span className={`px-2 py-1 rounded-full text-xs font-medium ${getCategoryStyle(course.category)}`}>
                    {course.category}
                  </span>
                </div>
                ...
                <div className="flex items-center justify-between pt-3 border-t border-gray-100">
                  <button
                    ...
                    className={`text-sm ${course.is_active ? "text-green-600" : "text-gray-400"}`}
                  >
