...
export default function SetupWizardPage() {
    ...
    const classList =
      type === "primary"
        ...
        : type === "secondary"
          ...
          : COMBINED_CLASSES;
    const subjectList =
      type === "secondary" ? SECONDARY_SUBJECTS : PRIMARY_SUBJECTS;
    const feeList =
      type === "secondary"
        ? DEFAULT_FEE_ITEMS.secondary
        : DEFAULT_FEE_ITEMS.primary;
  ...
  const addStaff = () =>
    setStaff((prev) => [...prev, { name: "", phone: "", role: "teacher" }]);
  ...
  const createStaffAccount = async (
    ...
    if (!response.ok || !result.success) {
      throw new Error(
        result.error || `Failed to create staff member "${member.name}"`,
      );
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const { error: schoolError } = await supabase
        ...
        .eq("id", schoolId);
      if (schoolError)
        throw new Error(`Failed to update school info: ${schoolError.message}`);
      ...
      if (classRows.length > 0) {
        ...
        if (classError)
          throw new Error(`Failed to save classes: ${classError.message}`);
      }
      ...
      if (subjectRows.length > 0) {
        ...
        if (subjectError)
          throw new Error(`Failed to save subjects: ${subjectError.message}`);
      }
      ...
      if (feeRows.length > 0) {
        ...
        if (feeError)
          throw new Error(`Failed to save fee structure: ${feeError.message}`);
      }

      const checklistItems = [];
      // Only include checklist items if corresponding data was written
      if (academic.year) {
        checklistItems.push({
      ...
      if (checklistItems.length > 0) {
        ...
        if (checklistError) {
          throw new Error(
            `Failed to update setup checklist: ${checklistError.message}`,
          );
        }
      }
      ...
      const { error: completeError } = await supabase
        ...
        .eq("id", schoolId);
      if (completeError)
        throw new Error(`Failed to finalize setup: ${completeError.message}`);
        ...
        return (
          <div className="space-y-5">
            ...
            <p className="text-sm text-[#5c6670]">
              Basic details about your school. This appears on reports,
              receipts, and the dashboard.
            </p>
            <div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setSchoolInfo({ ...schoolInfo, district: e.target.value })
                  }
                  className="input"
            ...
            <div>
              ...
              <div className="flex items-center gap-3">
                <input
                  type="color"
                  value={schoolInfo.primary_color}
                  onChange={(e) =>
                    setSchoolInfo({
                      ...schoolInfo,
                ...
                <span className="text-xs text-[#5c6670]">
                  Used in reports, receipts & dashboard
                </span>
              </div>
            </div>
          </div>
      ...
      case 2:
        return (
          <div className="space-y-5">
            ...
            <p className="text-sm text-[#5c6670]">
              Set up your school type, academic year, and grading system.
            </p>
            <div>
              ...
              <input
                ...
                value={academic.year}
                onChange={(e) =>
                  setAcademic({ ...academic, year: e.target.value })
                }
                className="input"
              />
            </div>
              ...
              <div className="grid grid-cols-3 gap-3">
                {(["primary", "secondary", "combined"] as const).map((type) => (
                  <button
                    ...
                    className={`p-4 rounded-xl border-2 text-center transition-all ${academic.type === type ? "border-blue-600 bg-blue-50" : "border-gray-200 hover:border-gray-300"}`}
                  >
                    <div className="font-medium text-[#002045] capitalize">
                      {type}
                    </div>
            ...
            <div>
              ...
              <select
                value={academic.terms}
                onChange={(e) =>
                  setAcademic({ ...academic, terms: parseInt(e.target.value) })
                }
                className="input"
              >
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setAcademic({
                    ...academic,
                    grading_system: e.target.value as any,
                  })
                }
                className="input"
              >
          ...
          <div className="space-y-5">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-[#5c6670]">
                  {classes.length} classes will be created
                </p>
              </div>
              <button
                onClick={() => {
                  const name = prompt("Class name:");
                  if (name) setClasses((prev) => [...prev, name]);
                }}
      ...
      case 4:
        return (
          <div className="space-y-5">
            ...
            <p className="text-sm text-[#5c6670]">
              Select subjects for your school. Compulsory subjects are
              pre-selected.
            </p>
            <div className="grid grid-cols-2 gap-2">
              {subjects.map((subject, i) => (
                <button
                  ...
                  className={`flex items-center gap-3 p-3 rounded-lg border-2 text-left transition-all ${subject.selected ? "border-blue-500 bg-blue-50" : "border-gray-200 opacity-60"}`}
                >
                  <div
                    className={`w-5 h-5 rounded border-2 flex items-center justify-center ${subject.selected ? "bg-blue-600 border-blue-600" : "border-gray-300"}`}
                  >
                    {subject.selected && (
                      <MaterialIcon
                        icon="check"
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-medium text-[#002045]">
                      {subject.name}
                    </div>
                    <div className="text-xs text-[#5c6670]">
                      {subject.code} {subject.compulsory && "• Compulsory"}
                    </div>
                  </div>
                </button>
              ))}
          ...
          <div className="space-y-5">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-[#5c6670]">
                  Add key staff members. You can add more later.
                </p>
              </div>
              <button onClick={addStaff} className="btn btn-secondary btn-sm">
                <MaterialIcon icon="add" /> Add
              </button>
            </div>
            {staff.length === 0 && (
              <div className="text-center py-8 text-[#5c6670]">
                ...
                />
                <p className="text-sm">
                  No staff added yet. Add at least a headteacher and bursar.
                </p>
                <button
                  onClick={addStaff}
            ...
            <div className="space-y-3">
              {staff.map((member, i) => (
                <div key={i} className="p-4 bg-[#f8fafb] rounded-xl space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-medium text-[#5c6670]">
                      Staff #{i + 1}
                    </span>
                    <button
          ...
          <div className="space-y-5">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-[#5c6670]">
                  Set up Term 1 fees. You can edit and add terms later.
                </p>
              </div>
              <button onClick={addFee} className="btn btn-secondary btn-sm">
                <MaterialIcon icon="add" /> Add
              </button>
            </div>
            <div className="space-y-2">
              {fees.map((fee, i) => (
                <div
                  key={i}
                  className="flex items-center gap-3 p-3 bg-[#f8fafb] rounded-lg"
                >
                  <input
                    type="text"
      ...
      case 7:
        return (
          <div className="space-y-5">
            ...
            <p className="text-sm text-[#5c6670]">
              You can import data now or do it later from the dashboard.
            </p>
            <div className="space-y-3">
              {[
                {
                  ...
                  icon: "group",
                  title: "Students",
                  desc: "Import student records from Excel/CSV",
                  badge: "Recommended",
                },
                {
                  key: "fees" as const,
                  icon: "payments",
                  title: "Fee Balances",
                  desc: "Import existing fee balances per student",
                  badge: "",
                },
                {
                  key: "skip" as const,
                  icon: "skip_next",
                  title: "Skip for Now",
                  desc: "I'll set up data later from the dashboard",
                  badge: "",
                },
              ].map((option) => (
                <button
                  key={option.key}
                  onClick={() => setImportChoice(option.key)}
                  className={`w-full flex items-center gap-4 p-4 rounded-xl border-2 text-left transition-all ${importChoice === option.key ? "border-blue-500 bg-blue-50" : "border-gray-200 hover:border-gray-300"}`}
                >
        ...
        return (
          <div className="space-y-6 text-center py-8">
            ...
            <p className="text-[#5c6670] max-w-md mx-auto">
              Your school is configured with {classes.length} classes,{" "}
              {subjects.filter((s) => s.selected).length} subjects, and{" "}
            ...
            {importChoice === "students" && (
              <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 max-w-md mx-auto text-left">
                ...
                <p className="text-sm text-amber-700">
                  You'll be redirected to the import page to upload your student
                  records.
                </p>
              </div>
            )}
            {importChoice === "fees" && (
              <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 max-w-md mx-auto text-left">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="font-medium text-amber-800">
                    Next: Import Fee Balances
                  </span>
                </div>
                <p className="text-sm text-amber-700">
                  You'll be redirected to import existing fee balances.
                </p>
              </div>
            )}
          </div>
      ...
      <div className="min-h-screen bg-[#f5f6f8]">
        ...
        <div className="bg-white border-b border-[#e8eaed] px-6 py-4">
          <div className="max-w-3xl mx-auto flex items-center justify-between">
            <div>
              <h1 className="text-lg font-bold text-[#002045]">
                School Setup Wizard
              </h1>
              <p className="text-sm text-[#5c6670]">
            ...
            <button
              ...
              className="text-sm text-[#5c6670] hover:text-[#002045]"
            >
              Skip to Dashboard
            </button>
          </div>
            ...
            <div className="flex items-center gap-1 overflow-x-auto">
              {STEPS.map((step, i) => (
                <div key={step.id} className="flex items-center flex-1 min-w-0">
                  <div
                    className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm whitespace-nowrap ${currentStep >= step.id ? "bg-blue-50 text-blue-700" : "text-gray-400"}`}
                  >
                    <div
                      className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${currentStep > step.id ? "bg-blue-600 text-white" : currentStep === step.id ? "bg-blue-600 text-white" : "bg-gray-200 text-gray-500"}`}
                    >
                      {currentStep > step.id ? (
                  ...
                  {i < STEPS.length - 1 && (
                    <div
                      className={`flex-1 h-0.5 mx-1 ${currentStep > step.id ? "bg-blue-600" : "bg-gray-200"}`}
                    />
                  )}
                </div>
              ))}
          ...
          <div className="bg-white rounded-2xl border border-[#e8eaed] p-6">
            ...
            <div className="flex gap-3 mt-8 pt-6 border-t border-gray-100">
              ...
              {currentStep < 7 ? (
                ...
                <button
                  onClick={() => {
                    if (importChoice === "students")
                      router.push("/dashboard/import?type=students");
                    else if (importChoice === "fees")
                      router.push("/dashboard/import?type=fees");
