...
export default function SettingsPage() {
  ...
  const [users, setUsers] = useState<
    Array<{ id: string; full_name: string; phone: string; role: string; is_active: boolean }>
  >([]);
  ...
  const [selectedPlan, setSelectedPlan] = useState<string>(
    school?.subscription_plan === "basic" ? "starter" : school?.subscription_plan || "starter",
  );
  ...
  useEffect(() => {
    if (school?.feature_stage) setSelectedStage(school.feature_stage as FeatureStage);
  }, [school?.feature_stage]);
  useEffect(() => {
    if (school?.id) fetchSettings();
  }, [school?.id, fetchSettings]);

  useEffect(() => {
    if (!school?.id) return;
    loadSchoolSettings(school.id)
      .then((settingsMap) => {
  ...
  const fetchUsers = useCallback(async () => {
    ...
    try {
      ...
      const result = await withTimeout(
        supabase.from("users").select("*").eq("school_id", school.id).order("created_at", { ascending: false }),
        10000,
        timeoutFallback(),
      );
      if (result.error) throw result.error;
      setUsers(
        (result.data || []) as { id: string; full_name: string; phone: string; role: string; is_active: boolean }[],
      );
    } catch (err) {
      logger.error("Error:", err);
    } finally {
  ...
  useEffect(() => {
    if (activeTab === "config" && school?.id) fetchHouses();
  }, [activeTab, school?.id, fetchHouses]);
  useEffect(() => {
    if (activeTab === "users" && school?.id) fetchUsers();
  ...
  const fetchModuleData = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok || !result.success) {
        throw new Error(typeof result.error === "string" ? result.error : "Failed to load modules");
      }
  ...
  const addHouse = async (name: string, color: string, motto: string) => {
    if (!school?.id || !name.trim()) {
      return;
    }
    try {
      const { withTimeout } = await import("@/lib/hooks/utils");
      const houseResult = await withTimeout(
        supabase.from("houses").insert({ school_id: school.id, name: name.trim(), color, motto: motto.trim() || null }),
        15000,
        timeoutFallback(),
      );
  ...
  const handlePlanUpgrade = async (plan: string) => {
    ...
    const normalizedPlan =
      plan === "basic" ? "starter" : plan === "premium" ? "growth" : plan === "max" ? "enterprise" : plan;
    if (normalizedPlan === "starter" || normalizedPlan === "free_trial") {
      setUpgradingPlan(true);
      try {
        const { error } = await supabase
          .from("schools")
          .update({ subscription_plan: normalizedPlan, subscription_status: "trial" })
          .eq("id", school.id);
        if (error) throw error;
        await refreshSchool();
        toast.success(`Successfully switched to ${normalizedPlan.toUpperCase()} plan!`);
      } catch (err: unknown) {
    ...
    setSelectedPaymentPlan(normalizedPlan);
    setShowPaymentModal(true);
  };
    ...
    try {
      if (provider === "mtn" || provider === "airtel") {
        ...
        const response = await fetch("/api/payment/mobile-money/", {
          ...
          body: JSON.stringify({ provider, plan: selectedPaymentPlan, phoneNumber: phone }),
        });
        const result = await parseApiResponse(response);
        if (!response.ok || result.error) {
          if (typeof result.contactWhatsApp === "string") setPaymentWhatsAppLink(result.contactWhatsApp);
          throw new Error(typeof result.error === "string" ? result.error : "Payment failed");
        }
        if (result.paymentLink) {
          toast.success(
            typeof result.instructions === "string"
              ? result.instructions
              : "A payment prompt has been sent to your phone. Enter your PIN to complete.",
          );
          setShowPaymentModal(false);
          setPaymentPhone("");
          setPaymentWhatsAppLink(null);
        ...
        const response = await fetch("/api/payment/checkout/", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ provider: "paypal", plan: selectedPaymentPlan }),
        });
        const result = await parseApiResponse(response);
        if (!response.ok || result.error) {
          if (typeof result.contactWhatsApp === "string") setPaymentWhatsAppLink(result.contactWhatsApp);
        ...
        if (typeof result.url === "string") {
          const target = result.url.trim();
          if (!target || target === "about:blank") throw new Error("PayPal returned an invalid payment link.");
          const newTab = window.open(target, "_blank", "noopener,noreferrer");
          if (!newTab) window.location.assign(target);
  ...
  const toggleUserStatus = async (id: string, currentStatus: boolean) => {
    try {
      const result = await withTimeout(
        supabase.from("users").update({ is_active: !currentStatus }).eq("id", id),
        15000,
        timeoutFallback(),
      );
      if (result?.error) throw result.error;
  ...
  const handleAddUser = (data: { full_name: string; phone: string; role: UserRole; password: string }) => {
    ...
    fetch("/api/users", {
      ...
      .then((result) => {
        if (!result.success) {
          throw new Error(typeof result.error === "string" ? result.error : "Failed to add user");
        }
  ...
  const exportStudentPhotos = async () => {
    ...
    try {
      ...
      if (result?.error) throw result.error;
      const photoManifest = (result.data || [])
        ...
        .filter((s: Record<string, unknown>) => Boolean(s.photo_url));
  ...
  const switchBillingMode = async (nextMode: "full_suite" | "modular") => {
    ...
    try {
      ...
      const result = await parseApiResponse(response);
      if (!response.ok || !result.success) {
        throw new Error(typeof result.error === "string" ? result.error : "Failed to switch billing mode");
      }
  ...
  const activateModule = async (moduleKey: ModuleKey) => {
    ...
    try {
      ...
      if (!response.ok || !result.success) {
        throw new Error(typeof result.error === "string" ? result.error : "Failed to activate module");
      }
            ...
            <Card>
              <CardBody className="p-5 space-y-4">
                ...
                <p className="text-sm text-[var(--t3)]">
                  Set a custom support phone number for your school. When payment or technical issues arise, users will
                  be directed to this number on WhatsApp. Leave empty to use the platform default.
                </p>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-blue-700 mt-1">
                    Use this checklist to ensure your school is fully configured. Items can be completed now or skipped
                    for later. We'll remind you to complete them.
                  </p>
                </div>
              </div>
            </div>
                  ...
                  <div className="p-4 bg-[var(--blue-soft)] rounded-xl border border-blue-200">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="text-sm text-blue-900">
                        ...
                        <p className="mt-1">
                          If your installed app does not auto-update, download the latest build below and reinstall.
                  ...
                  <div className="p-4 bg-[var(--surface-container-low)] rounded-xl">
                    <div className="flex items-center justify-between">
                      ...
                      <Button onClick={exportAllData}>
                        <MaterialIcon icon="download" className="text-lg" /> Export
                      </Button>
                    </div>
                  </div>
                  <div className="p-4 bg-[var(--surface-container-low)] rounded-xl">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-sm text-[var(--t3)]">
                          Regular backups are recommended. Cloud backup is available on Premium plans.
                        </div>
                      </div>
                    </div>
                  </div>
          ...
          <div className="space-y-6">
            {!isSupabaseConfigured && (
              <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
                Module and billing actions are disabled in mock mode. Add real Supabase credentials in{" "}
                <code>.env.local</code> to run live billing tests.
              </div>
            )}
              ...
              <CardBody>
                <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between mb-4">
                  <div>
                    ...
                    <p className="text-sm text-[var(--t3)]">
                      Get all modules included, or pick only the ones your school needs.
                    </p>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="p-6 rounded-2xl border-2 border-[var(--border)] bg-[var(--surface-container-low)]">
                    ...
                    <div className="text-2xl font-bold mb-4">
                      UGX 250k <span className="text-sm font-normal text-[var(--t3)]">/ term</span>
                    </div>
                    <ul className="space-y-3 mb-6">
                      <li className="flex items-center gap-2 text-sm">
                        <MaterialIcon icon="check_circle" className="text-[var(--green)] text-base" /> Up to 100
                        Students
                      </li>
                      <li className="flex items-center gap-2 text-sm">
                        <MaterialIcon icon="check_circle" className="text-[var(--green)] text-base" /> Basic Attendance
                      </li>
                      <li className="flex items-center gap-2 text-sm">
                        <MaterialIcon icon="check_circle" className="text-[var(--green)] text-base" /> Fee Management
                      </li>
                    </ul>
                    <Button
                      variant="secondary"
                  ...
                  <div className="p-6 rounded-2xl border-2 border-[var(--primary)] bg-[var(--primary-soft)] relative">
                    ...
                    <div className="text-2xl font-bold mb-4">
                      UGX 3,500 <span className="text-sm font-normal text-[var(--t3)]">/ month</span>
                    </div>
                    <ul className="space-y-3 mb-6">
                      <li className="flex items-center gap-2 text-sm">
                        <MaterialIcon icon="check_circle" className="text-[var(--green)] text-base" /> Up to 500
                        Students
                      </li>
                      <li className="flex items-center gap-2 text-sm">
                        <MaterialIcon icon="check_circle" className="text-[var(--green)] text-base" /> SMS Notifications
                      </li>
                      <li className="flex items-center gap-2 text-sm">
                        <MaterialIcon icon="check_circle" className="text-[var(--green)] text-base" /> Report Card
                        Printing
                      </li>
                    </ul>
                    <Button
                    ...
                    >
                      {(school?.subscription_plan as string) === "growth" ||
                      (school?.subscription_plan as string) === "premium"
                        ? "Current Plan"
                        : "Select"}
                    </Button>
                  </div>
                  <div className="p-6 rounded-2xl border-2 border-[var(--border)] bg-[var(--surface-container-low)]">
                    ...
                    <div className="text-2xl font-bold mb-4">
                      UGX 5,500 <span className="text-sm font-normal text-[var(--t3)]">/ month</span>
                    </div>
                    <ul className="space-y-3 mb-6">
                      <li className="flex items-center gap-2 text-sm">
                        <MaterialIcon icon="check_circle" className="text-[var(--green)] text-base" /> Unlimited
                        Students
                      </li>
                      <li className="flex items-center gap-2 text-sm">
                        <MaterialIcon icon="check_circle" className="text-[var(--green)] text-base" /> AI Smart Advisor
                      </li>
                      <li className="flex items-center gap-2 text-sm">
                        <MaterialIcon icon="check_circle" className="text-[var(--green)] text-base" /> Full Payroll &
                        Assets
                      </li>
                    </ul>
                    <Button
                    ...
                    >
                      {(school?.subscription_plan as string) === "enterprise" ||
                      (school?.subscription_plan as string) === "max"
                        ? "Current Plan"
                        : "Upgrade"}
                    </Button>
                  </div>
                  ...
                  <div className="flex gap-4 p-4 rounded-xl bg-[var(--surface-container)]">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-[var(--t3)]">
                        Get proactive alerts about performance drops, fee deficits, and staff workload optimization.
                      </div>
                    </div>
                  </div>
                  <div className="flex gap-4 p-4 rounded-xl bg-[var(--surface-container)]">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-[var(--t3)]">
                        Recover fees 3.5x faster with automatic, personalized SMS nudges to parents.
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <Card>
              <CardBody>
                <div className="flex items-center justify-between mb-4">
                  ...
                  {paymentHistory.length > 0 && <span className="text-[11px] text-[var(--t3)]">Last 10 payments</span>}
                </div>
                {paymentHistory.length === 0 ? (
                  <div className="text-sm text-[var(--t3)] py-4 text-center">No payment history yet.</div>
                ) : (
            ...
            <div className="bg-white rounded-2xl p-6 max-w-md w-full max-h-[calc(100vh-1.5rem)] sm:max-h-[calc(100vh-2rem)] overflow-y-auto my-auto shadow-2xl">
              ...
              <div className="mb-6">
                <p className="text-[var(--t3)] mb-2">
                  You selected <strong>{selectedPaymentPlan?.toUpperCase()}</strong> plan
                </p>
              ...
              <div className="space-y-3">
                ...
                >
                  ...
                  <div className="text-left">
                    <div className="font-semibold text-[var(--on-surface)]">MTN Mobile Money</div>
                    <div className="text-xs text-[var(--t3)]">Instant prompt on your phone</div>
                  </div>
                </button>
                <button
                ...
                >
                  <span className="text-2xl">🔴</span>
                  <div className="text-left">
                    <div className="font-semibold text-[var(--on-surface)]">Airtel Money</div>
                    <div className="text-xs text-[var(--t3)]">Instant prompt on your phone</div>
                  </div>
                </button>
                <button
                ...
                >
                  <span className="text-2xl">💳</span>
                  <div className="text-left">
                    <div className="font-semibold text-[var(--on-surface)]">PayPal</div>
                    <div className="text-xs text-[var(--t3)]">International cards accepted</div>
                  </div>
                </button>
              </div>
              {paymentWhatsAppLink && (
                <div className="mt-4 p-4 rounded-xl bg-red-50 border border-red-200">
                  <p className="text-sm font-medium text-red-700 mb-2">Online payment is currently unavailable</p>
                  <p className="text-xs text-red-600 mb-3">
                    You can complete your payment by contacting us on WhatsApp. Our team will guide you through the
                    process.
                  </p>
                  <a
