...
export default function SettingsPage() {
  ...
  const [selectedPlan, setSelectedPlan] = useState<string>(school?.subscription_plan === "basic" ? "starter" : school?.subscription_plan || "starter");
  ...
  const handlePlanUpgrade = async (plan: string) => {
    if (!school?.id) return;
    const normalizedPlan = plan === "basic" ? "starter" : plan === "premium" ? "growth" : plan === "max" ? "enterprise" : plan;
    if (normalizedPlan === "starter" || normalizedPlan === "free_trial") {
      setUpgradingPlan(true);
      try {
        const { error } = await supabase.from("schools").update({ subscription_plan: normalizedPlan, subscription_status: "trial" }).eq("id", school.id);
          ...
          <Card>
            <CardBody>
              <div className="flex items-center justify-between mb-6">
                ...
                <div className="px-4 py-2 bg-[var(--green-soft)] text-[var(--green)] rounded-full text-sm font-semibold">{school?.subscription_plan ? `${school.subscription_plan.toUpperCase()} PLAN` : "ACTIVE"}</div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="p-6 rounded-2xl border-2 border-[var(--primary)] bg-[var(--primary-soft)] relative">
                  ...
                  <Button className="w-full" loading={upgradingPlan && selectedPlan === "growth"} onClick={() => handlePlanUpgrade("growth")}>{(school?.subscription_plan as string) === "growth" || (school?.subscription_plan as string) === "premium" ? "Current Plan" : "Select"}</Button>
                </div>
                <div className="p-6 rounded-2xl border-2 border-[var(--border)] bg-[var(--surface-container-low)]">
                  ...
                  <Button variant="secondary" className="w-full" loading={upgradingPlan && selectedPlan === "enterprise"} onClick={() => handlePlanUpgrade("enterprise")}>{(school?.subscription_plan as string) === "enterprise" || (school?.subscription_plan as string) === "max" ? "Current Plan" : "Upgrade"}</Button>
                </div>
              </div>
            </CardBody>
          </Card>
