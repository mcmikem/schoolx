...
export default function BudgetPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const { data: feeData } = await withTimeout(
        supabase
          .from("fee_structure")
          .select("amount")
          .eq("school_id", school.id)
      ...
      const { data: budgetData } = await withTimeout(
        supabase
          .from("budget_items")
          .select("*")
          .eq("school_id", school.id)
      ...
      const { data: expenses } = await withTimeout(
        supabase
          .from("expenses")
      ...
      const { data: fees } = await withTimeout(
        supabase
          .from("fee_payments")
                        ...
                        <div className="flex-1">
                          ...
                          {cat.auto && (
                            <div className="text-xs text-[var(--t3)]">
                              {studentCount} students × UGX {feePerTerm.toLocaleString()}
                            </div>
                          )}
                        </div>
                        <input
          ...
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 mb-6">
            ...
            <Card className="text-center">
              <CardBody>
                ...
                <div
                  className={`text-xl font-bold ${
                    totalExpense > 0
                      ? (
                          () => {
                            const p = (actualExpense / totalExpense) * 100;
                            if (p > 90) return "text-[var(--error)]";
                            if (p > 75) return "text-[var(--amber)]";
                            return "text-[var(--green)]";
                          }
                        )()
                      : "text-[var(--t3)]"
                ...
                >
                  {totalExpense > 0 ? `${Math.round((actualExpense / totalExpense) * 100)}%` : "N/A"}
                </div>
              </CardBody>
            </Card>
          </div>
              ...
              <table className="w-full">
                <thead className="bg-[var(--surface-container)]">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-right text-sm font-medium text-[var(--t2)]">Variance</th>
                      ...
                      <tr key={cat.key} className="border-b border-[var(--border)]">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center justify-center gap-2">
                            ...
                            <span className={`text-sm font-medium ${getUsageColor(item.budgeted, item.actual)}`}>
                              {pct}%
                            </span>
                          </div>
                        </td>
