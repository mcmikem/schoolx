...
export default function ConductManagementPage() {
  const [activeType, setActiveType] = useState<"all" | "merit" | "demerit">("all");

  const mockRecords = [
    { id: 1, student: "Isaac Mugisha", class: "P.5 Blue", type: "merit", category: "Leadership", description: "Led the class assembly with great confidence.", points: 10, date: "2026-04-09" },
    { id: 2, student: "Sarah Namayanja", class: "P.4 Red", type: "demerit", category: "Late Coming", description: "Arrived significantly late for the third time this week.", points: -5, date: "2026-04-08" },
    { id: 3, student: "Emma Kato", class: "P.6 Blue", type: "merit", category: "Academic Excellence", description: "Achieved the highest score in the regional math competition.", points: 20, date: "2026-04-07" },
  ];
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-black text-slate-800 tracking-tight">Conduct & Merits</h1>
          <p className="text-slate-500 font-medium">Character tracking and disciplinary management</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="p-6 rounded-[32px] bg-emerald-50 border border-emerald-100 flex items-center gap-4">
           ...
           <div>
             <p className="text-[10px] font-black uppercase tracking-widest text-emerald-700">Monthly Merits</p>
        ...
        <div className="p-6 rounded-[32px] bg-primary-50 border border-primary-100 flex items-center gap-4">
           ...
           <div>
             <p className="text-[10px] font-black uppercase tracking-widest text-primary-700">Leading House</p>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
             ...
             <tbody className="divide-y divide-slate-50">
                {mockRecords.filter(r => activeType === 'all' || r.type === activeType).map(record => (
                   <tr key={record.id} className="hover:bg-slate-50/50 transition-colors group">
                      ...
                      <td className="px-6 py-4">
                         <div className="flex items-center gap-3">
                            ...
                            <div>
                               <p className="text-sm font-bold text-slate-800">{record.student}</p>
                               <p className="text-[10px] font-bold text-slate-400">{record.class}</p>
                            </div>
                         </div>
                      </td>
                      <td className="px-6 py-4">
                         <span className={`px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-widest ${
                            record.type === 'merit' ? "bg-emerald-50 text-emerald-600 border border-emerald-100" : "bg-red-50 text-red-600 border border-red-100"
                         }`}>
                            {record.category}
                         </span>
                      </td>
                      ...
                      <td className="px-6 py-4">
                         <p className={`text-sm font-black ${record.type === 'merit' ? 'text-emerald-600' : 'text-red-600'}`}>
