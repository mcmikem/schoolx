...
export default function ClassesPage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingClass) {
        await updateRecord(() => supabase.from("classes").update(payload).eq("id", editingClass.id), {
          timeoutMs: 8000,
          timeoutMessage: "Update timed out — please try again",
        });
        toast.success("Class updated");
      } else {
        await createRecord(() => supabase.from("classes").insert(payload), {
          timeoutMs: 8000,
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      await deleteRecord(() => supabase.from("classes").delete().eq("id", pendingDelete.id), {
        timeoutMs: 8000,
  ...
  const years = Array.from(new Set(classes.map((c) => c.academic_year)))
    .sort()
    .reverse();
    ...
    <PageErrorBoundary>
      <div className="p-4 sm:p-6 lg:p-8 max-w-6xl mx-auto space-y-6">
          ...
          {[
            ...
            <Card key={s.label} className="!bg-[var(--surface-container-low)]">
              <CardBody className="flex items-center gap-3">
                <MaterialIcon
                  icon={s.icon}
                  className={`text-2xl ${s.warn ? "text-amber-500" : "text-[var(--primary)]"}`}
                />
                <div>
                  <p className="text-xs text-[var(--on-surface-variant)]">{s.label}</p>
                  <p className={`text-xl font-bold ${s.warn ? "text-amber-600" : "text-[var(--on-surface)]"}`}>
                    {s.value}
                  </p>
                </div>
              </CardBody>
            ...
            {loading ? (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-[var(--border)] text-left">
                    {["Class Name", "Level", "Stream", "Class Teacher", "Students", "Capacity", ""].map((h) => (
                      <th
                        key={h}
                        className="px-4 py-3 text-xs font-semibold text-[var(--on-surface-variant)] uppercase tracking-wider whitespace-nowrap"
                      >
                        {h}
                      </th>
                    ))}
                ...
                <tbody className="divide-y divide-[var(--border)]">
                  {filtered.map((cls) => {
                    const pct =
                      cls.max_students > 0 ? Math.round(((cls.student_count || 0) / cls.max_students) * 100) : 0;
                    ...
                    return (
                      <tr key={cls.id} className="hover:bg-[var(--surface-container-low)] transition-colors">
                        ...
                        <td className="px-4 py-3 text-[var(--on-surface-variant)]">
                          {(cls.teacher as { full_name: string } | null)?.full_name || (
                            <span className="italic text-[var(--outline)]">Unassigned</span>
                          )}
                        </td>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            <div className="w-20 h-1.5 bg-[var(--surface-container-highest)] rounded-full overflow-hidden">
                              <div
                                className={`h-full rounded-full ${isFull ? "bg-red-500" : pct > 80 ? "bg-amber-400" : "bg-[var(--primary)]"}`}
                                style={{ width: `${Math.min(pct, 100)}%` }}
                              />
                            </div>
                            <span
                              className={`text-xs font-medium ${isFull ? "text-red-600" : "text-[var(--on-surface-variant)]"}`}
                            >
        ...
        {showModal && (
          ...
          >
            <div
              className="bg-[var(--surface)] rounded-2xl w-full max-w-md max-h-[calc(100vh-1.5rem)] sm:max-h-[calc(100vh-2rem)] overflow-y-auto my-auto shadow-2xl"
        ...
        <ConfirmDialog
          ...
          message={
            pendingDelete
              ? `Delete "${pendingDelete.name}"? This cannot be undone. Make sure no students are enrolled in this class first.`
              : ""
          }
