      ...
      studentMap.forEach((student) => {
        if (student.subjects.length > 0) {
          ...
          if (examType === "uace") {
            ...
            student.grade = examType === "uce" ? getUCEGrade(avg) : getGradeForLevel(avg, "primary", gradeLabels);
          }
    ...
    <PageErrorBoundary>
      <div className="p-4 sm:p-6 lg:p-8 space-y-6">
        ...
        {loading ? (
                      ...
                      {results.map((student, i) => (
                        <tr key={student.id} className="border-t border-[var(--border)]">
                          ...
                          <td className="p-3">
                            <span
                              className={`font-bold ${
                                student.grade.startsWith("D") || student.grade === "A"
                                  ? "text-[var(--green)]"
                                  : student.grade.startsWith("C") || student.grade === "B"
                                    ? "text-[var(--navy)]"
                                    : student.grade.startsWith("P") || student.grade === "C"
                                      ? "text-[var(--amber)]"
                                      : "text-[var(--red)]"
                              }`}
                            >
          ...
          <EmptyState icon="school" title="No exam results found" description="Enter exam scores first." />
        )}
      </div>
    </PageErrorBoundary>
  );
