    ...
    <div className="space-y-6">
        ...
        actions={
          <div className="flex items-center gap-3">
            <div className="flex flex-col items-end mr-2">
              ...
              <span className="text-sm font-black text-amber-600 uppercase tracking-tighter italic">
                ★ {tier} Partner
              </span>
            </div>
            <Button
              variant="secondary"
              size="sm"
              icon={<MaterialIcon icon="upgrade" />}
            >
              Upgrade Tier
            </Button>
          </div>
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-8 max-w-4xl mx-auto">
            {STEPS.map((step, idx) => (
              ...
              >
                <div
                  className={`w-12 h-12 rounded-full flex items-center justify-center font-bold text-sm mb-2 transition-all shadow-md ${
                    step.status === "complete"
                      ...
                      : step.status === "current"
                        ? "bg-white text-[var(--navy)] ring-4 ring-white/30 animate-pulse"
                ...
                {idx < STEPS.length - 1 && (
                  <div
                    className={`hidden md:block absolute top-6 left-[calc(50%+28px)] w-[calc(100%-56px)] h-0.5 ${step.status === "complete" ? "bg-green-500/30" : "bg-[#002045]/10"}`}
                  />
                )}
              </div>
            ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2 space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {PROGRAMS.map((prog) => (
              <Card
                key={prog.id}
                className={`group hover:shadow-xl transition-all duration-300 border-l-4 ${prog.active ? "border-primary" : "border-gray-200 opacity-70"}`}
              >
                <CardBody>
                  <div className="flex items-start justify-between mb-3 text-sm">
                    <div
                      className={`w-10 h-10 rounded-xl flex items-center justify-center text-white shadow-lg ${prog.color}`}
                    >
                      <MaterialIcon icon={prog.icon} />
                    </div>
                    <span
                      className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${prog.active ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"}`}
                    >
                      {prog.active ? "Active" : "Inactive"}
                    </span>
                  </div>
                  <h3 className="font-bold text-[var(--t1)] mb-1">
                    {prog.name}
                  </h3>
                  <p className="text-xs text-[var(--t3)] leading-relaxed mb-4">
                    {prog.description}
                  </p>
                  <div className="flex items-center justify-between">
                    ...
                    <Button
                      variant="ghost"
                      size="sm"
                      className="p-1 min-h-0 text-[10px] uppercase font-bold text-[var(--t4)] hover:text-primary"
                    >
                      Reports ↗
                    </Button>
                  </div>
        ...
        <div className="space-y-6">
          ...
          <Card>
            <CardBody>
              <h3 className="font-bold text-sm text-[var(--t1)] mb-4 uppercase tracking-tighter">
                Foundation Services
              </h3>
                ...
                {[
                  {
                    name: "Exam Printing",
                    icon: "print",
                    desc: "Starting UGX 300/pg",
                  },
                  {
                    name: "Branded Uniforms",
                    icon: "apparel",
                    desc: "Quality tailoring",
                  },
                  {
                    name: "Event Coverage",
                    icon: "photo_camera",
                  ...
                  },
                  {
                    name: "Design & Signage",
                    icon: "draw",
                    desc: "School profiling",
                  },
                ].map((svc) => (
                  <button
                    key={svc.name}
                    className="w-full h-auto p-3 text-left bg-gray-50 hover:bg-gray-100 rounded-xl transition-all border border-transparent hover:border-gray-200"
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="text-[12px] font-bold text-[var(--t1)]">
                          {svc.name}
                        </div>
                        <div className="text-[10px] text-[var(--t4)] font-medium">
                          {svc.desc}
                        </div>
                      </div>
                    </div>
              ...
              <div className="mt-4 p-3 bg-primary-50 rounded-xl border border-primary-100 text-[10px] text-primary-800 font-medium leading-relaxed">
                💡 When your school buys SkoolMate services, you are directly
                funding local youth employment.
              </div>
              <Button
                variant="primary"
                className="w-full mt-4 text-xs font-black uppercase tracking-widest py-3"
              >
                Request Rate Card
              </Button>
            </CardBody>
