...
export default function BoardReportPage() {
  ...
  const stats = useMemo(() => {
    ...
    const totalExpected =
      feeStructure.reduce((sum, f) => sum + Number(f.amount), 0) *
  ...
  const generatePDF = async () => {
    try {
      ...
      doc.setFontSize(10);
      doc.text(`Term ${currentTerm}, ${academicYear}`, 105, 33, {
        align: "center",
      });
      ...
      doc.text(
        `Boys: ${stats.boys} (${Math.round((stats.boys / stats.totalStudents) * 100)}%)`,
        20,
        86,
      );
      doc.text(
        `Girls: ${stats.girls} (${Math.round((stats.girls / stats.totalStudents) * 100)}%)`,
        20,
        94,
      );
      ...
      doc.setFontSize(11);
      doc.text(
        `Total Expected Revenue: ${formatCurrency(stats.totalExpected)}`,
        20,
        133,
      );
      doc.text(
      ...
      doc.text(
        `Outstanding Balance: ${formatCurrency(stats.totalExpected - stats.totalCollected)}`,
        20,
        149,
      );
      ...
      doc.setFontSize(8);
      doc.text(
        "This report is confidential and intended for board members only.",
        105,
        283,
        { align: "center" },
      );
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-6">
      ...
      <Card className="max-w-2xl">
        <CardBody>
          <h2 className="font-semibold text-[var(--t1)] mb-6">
            Report Preview
          </h2>

          <div className="space-y-6">
            <div className="p-4 bg-[var(--surface-container-low)] rounded-lg">
              <h3 className="font-medium text-[var(--t1)] mb-3">
                1. Enrollment
              </h3>
      ...
      <Card className="max-w-2xl">
        <CardBody>
          ...
          <ul className="space-y-2 text-sm text-[var(--t3)]">
            <li>
              Board reports provide a one-page executive summary for school
              owners and board members
            </li>
            <li>
              Reports include enrollment, financial, and academic performance
              data
            </li>
            <li>
              Generated reports are confidential and should only be shared with
