...
const TEMPLATES: MessageTemplate[] = [
  {
    ...
    icon: "payments",
    category: "Finance",
    body: "Dear Parent, this is a reminder that school fees for {student_name} are outstanding. Please clear UGX {amount} by {deadline}. Thank you.",
  },
  {
    id: "absentee",
    label: "Absent Alert",
    icon: "event_busy",
    category: "Attendance",
    body: "Dear Parent, your child {student_name} was absent today ({date}). Please contact the school if this was unplanned. \u2014 {school_name}",
  },
  {
    id: "exam_notice",
    label: "Exam Notice",
    icon: "fact_check",
    category: "Academic",
    body: "Dear Parent, {student_name}'s end-of-term examinations begin on {date}. Ensure your child is prepared and school fees are cleared. \u2014 {school_name}",
  },
  {
    id: "report_ready",
    ...
    icon: "description",
    category: "Academic",
    body: "Dear Parent, {student_name}'s report card for Term {term} is ready. Please collect it from the school office or view it on the Parent Portal. \u2014 {school_name}",
  },
  {
    id: "visitation",
    ...
    icon: "groups",
    category: "Events",
    body: "Dear Parent, Visitation Day is on {date}. You are welcome to visit and meet your child's teachers from 9AM to 3PM. \u2014 {school_name}",
  },
...
const SEGMENT_OPTIONS: { value: Segment; label: string; icon: string; description: string }[] = [
  ...
  {
    value: "class",
    label: "Parents of a Specific Class",
    icon: "school",
    description: "Target parents by their child\u2019s class",
  },
  {
    value: "overdue",
    label: "Parents with Overdue Fees",
    icon: "payments",
    description: "Parents whose children have unpaid fees",
  },
  {
    value: "balance",
    label: "Outstanding Balance > Amount",
    icon: "trending_up",
    description: "Parents with balance above a threshold",
  },
  {
    value: "student",
    label: "Specific Student",
    icon: "person_search",
    description: "Send to a particular student\u2019s parent",
  },
];
...
export default function SMSCenterPage() {
  ...
  const fetchParentPhones = useCallback(
    async (countOnly = false): Promise<{ phone: string; full_name: string }[]> => {
      if (!school?.id) return [];
    ...
    const timer = setTimeout(async () => {
      const data = await withTimeout(
        supabase
          ...
          .eq("school_id", school.id)
          .or(
            `first_name.ilike.%${studentQuery}%,last_name.ilike.%${studentQuery}%,student_number.ilike.%${studentQuery}%`,
          )
          .limit(10)
          .then((r) => r.data),
        5000,
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-4">
            ...
            {TEMPLATES.map((t) => (
              ...
              >
                <div
                  className={`w-10 h-10 rounded-2xl flex items-center justify-center shrink-0 ${activeTemplate.id === t.id ? "bg-white/10" : "bg-slate-50"}`}
                >
                ...
                <div className="min-w-0">
                  ...
                  <p
                    className={`text-[10px] font-bold uppercase tracking-widest ${activeTemplate.id === t.id ? "opacity-50" : "text-slate-400"}`}
                  >
                    {t.category}
                  </p>
                </div>
                ...
                {segment === "balance" && (
                  <div>
                    <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">
                      Minimum Outstanding Balance (UGX)
                    </p>
                    <input
                      type="number"
                      inputMode="numeric"
                    ...
                    {selectedStudent ? (
                      ...
                      <>
                        <input
                          ...
                          onFocus={() => {
                            if (studentResults.length > 0) setShowStudentDropdown(true);
                          }}
                ...
                {recipientCount !== null && (
                  <div className="flex items-center gap-3 px-4 py-3 rounded-2xl bg-indigo-50 border border-indigo-100">
                    ...
                    <p className="text-sm font-bold text-indigo-800">
                      SMS will be sent to <span className="text-indigo-600">{recipientCount}</span> parent
                      {recipientCount !== 1 ? "s" : ""}
                    </p>
                  </div>
                )}
              ...
              <CardBody className="p-6 space-y-4">
                <div className="flex justify-between items-center">
                  ...
                  <div className="flex items-center gap-3">
                    <span className={`text-[10px] font-bold ${charCount > 160 ? "text-amber-500" : "text-slate-400"}`}>
                      {charCount} chars &middot; {smsCount} SMS
                    </span>
                  </div>
                </div>
                <textarea
                ...
                <p className="text-[10px] text-slate-400 italic">
                  Variables: {"{student_name}"}, {"{date}"}, {"{amount}"}, {"{school_name}"}
                </p>
                  ...
                  <div className="divide-y divide-slate-50">
                    {logs.slice(0, 8).map((log) => (
                      <div key={log.id} className="px-5 py-4 flex items-center gap-4">
                        <div
                          className={`w-2 h-2 rounded-full shrink-0 ${log.status === "sent" ? "bg-emerald-400" : "bg-red-400"}`}
                        />
