...
export default function ParentPortal() {
  ...
  const getGradeColor = (grade: string) => {
    if (grade?.startsWith("D")) return "bg-emerald-100 text-emerald-700 border-emerald-200";
    if (grade?.startsWith("C")) return "bg-blue-100 text-blue-700 border-blue-200";
    if (grade?.startsWith("P")) return "bg-amber-100 text-amber-700 border-amber-200";
  ...
  if (!linkedStudent) {
    return (
      <div className="p-4 sm:p-6 lg:p-8 max-w-4xl mx-auto">
        <div className="mb-8">
          <h1 className="text-3xl font-black text-[var(--t1)] tracking-tight">Parent Portal</h1>
          <p className="text-[var(--t3)] mt-2">Welcome! No children are currently linked to your account.</p>
        </div>
        <div className="glass-premium rounded-3xl p-12 text-center">
          <MaterialIcon icon="family_restroom" className="text-6xl text-[var(--border)] mb-4" />
          <h3 className="text-xl font-bold text-[var(--t1)]">No Linked Students</h3>
          <p className="text-[var(--t3)] mt-2 max-w-md mx-auto">
            Please contact the school administration to link your child&apos;s academic profile to your parent account.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto w-full animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6 mb-8">
        <div>
          <h1 className="text-3xl font-black text-[var(--t1)] tracking-tight mb-2">Parent Portal</h1>
          <p className="text-[var(--t2)] text-base">Comprehensive academic and financial overview.</p>
        </div>
        ...
        {linkedStudents.length > 1 && (
          <div className="w-full sm:w-auto">
            ...
            <div className="relative">
              ...
              <MaterialIcon icon="expand_more" className="absolute right-3 top-1/2 -translate-y-1/2 text-[var(--t3)] pointer-events-none" />
            </div>
          </div>
        )}
      </div>
      ...
      <div className="glass-premium rounded-3xl p-6 sm:p-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 mb-8 relative overflow-hidden group">
        ...
        <div className="flex items-center gap-6 relative z-10">
          ...
          <div>
            <div className="text-2xl font-black text-[var(--t1)] tracking-tight">{studentName}</div>
            <div className="flex flex-wrap items-center gap-3 mt-2 text-sm text-[var(--t2)] font-medium">
              <span className="bg-[var(--surface-container)] px-3 py-1 rounded-full border border-[var(--border)]">{linkedStudent.classes?.name || "Unassigned"}</span>
              <span className="bg-[var(--surface-container)] px-3 py-1 rounded-full border border-[var(--border)]">{linkedStudent.student_number || "No ID"}</span>
              <span className="text-[var(--t3)]">•</span>
              <span className="text-[var(--primary)] font-bold">Term {currentTerm}, {academicYear}</span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 w-full sm:w-auto relative z-10">
           <div className="bg-[var(--surface-container)] p-4 rounded-2xl border border-[var(--border)] text-center min-w-[120px]">
             <p className="text-[10px] font-black uppercase text-[var(--t3)] tracking-widest mb-1">Attendance</p>
             <p className={`text-2xl font-black ${attendanceRate >= 90 ? "text-emerald-600" : attendanceRate >= 75 ? "text-amber-500" : "text-red-500"}`}>{attendanceRate}%</p>
           </div>
           <div className="bg-[var(--surface-container)] p-4 rounded-2xl border border-[var(--border)] text-center min-w-[120px]">
             <p className="text-[10px] font-black uppercase text-[var(--t3)] tracking-widest mb-1">Avg Grade</p>
             <p className="text-2xl font-black text-[var(--primary)]">{avgGrade}%</p>
           </div>
        </div>
      </div>
        ...
        <div className="space-y-8">
          ...
          <section className="glass-premium rounded-3xl p-6 sm:p-8 border-t-4 border-t-[var(--primary)]">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-lg font-black text-[var(--t1)] flex items-center gap-2">
                <MaterialIcon icon="account_balance" className="text-[var(--primary)]" />
                Tuition & Fees Status
              </h2>
              <span className={`px-3 py-1 rounded-full text-[10px] uppercase font-black tracking-widest border ${
                  feeStats.status === "cleared" ? "bg-emerald-50 text-emerald-600 border-emerald-200" 
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
               <div>
                  <p className="text-[10px] font-black uppercase text-[var(--t3)] tracking-widest mb-1">Total Paid</p>
                  <p className="text-xl font-bold text-[var(--t1)]">UGX {feeStats.totalPaid.toLocaleString()}</p>
               </div>
               <div className="text-right">
                  <p className="text-[10px] font-black uppercase text-[var(--t3)] tracking-widest mb-1">Total Fee</p>
                  <p className="text-xl font-bold text-[var(--t1)]">UGX {feeStats.totalFee.toLocaleString()}</p>
               </div>
            </div>
            ...
            <div className="h-4 bg-[var(--surface-container)] rounded-full overflow-hidden mb-6 relative border border-[var(--border)]">
              <div 
                ...
                style={{ width: `${feeStats.totalFee > 0 ? Math.min((feeStats.totalPaid / feeStats.totalFee) * 100, 100) : 0}%` }} 
              >
            ...
            <div className="bg-red-50/50 border border-red-100 rounded-2xl p-4 flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-black text-red-600">UGX {feeStats.balance.toLocaleString()}</p>
              </div>
          ...
          <section className="glass-premium rounded-3xl p-6 sm:p-8">
             ...
             <div className="flex items-center justify-between bg-emerald-50/50 border border-emerald-100 rounded-2xl p-5 mb-6">
                <div>
                  <p className="text-[10px] font-black uppercase tracking-widest text-emerald-600 mb-1">Available Balance</p>
                  <p className="text-3xl font-black text-emerald-700">UGX {Number(linkedStudent.balance || 0).toLocaleString()}</p>
                </div>
             ...
             <div>
               <h3 className="text-xs font-black uppercase tracking-widest text-[var(--t3)] mb-4">Recent Canteen Purchases</h3>
               {canteenSales.length === 0 ? (
                 <p className="text-sm text-[var(--t3)] italic text-center py-4 bg-[var(--surface-container-low)] rounded-xl border border-[var(--border)]">No recent purchases.</p>
               ) : (
                 <div className="space-y-3">
                   {canteenSales.map((sale) => (
                     <div key={sale.id} className="flex items-center justify-between p-3 border border-[var(--border)] rounded-xl hover:bg-[var(--surface-container-low)] transition-colors">
                       <div className="flex items-center gap-3">
                         <div className="w-10 h-10 rounded-lg bg-[var(--surface-container)] flex items-center justify-center text-[var(--primary)]">
                           <MaterialIcon icon="storefront" style={{fontSize: 20}} />
                         </div>
                         <div>
                           <p className="text-sm font-bold text-[var(--t1)]">{new Date(sale.created_at).toLocaleDateString("en-UG", { weekday: 'short', month: 'short', day: 'numeric' })}</p>
                           <p className="text-[10px] uppercase font-bold text-[var(--t4)] tracking-widest">
                             {sale.items?.length || 1} Item(s)
                           </p>
                         </div>
                       </div>
                       <p className="text-sm font-black text-red-500">- UGX {Number(sale.total_amount).toLocaleString()}</p>
                     </div>
                   ))}
                 </div>
               )}
        ...
        <div className="space-y-8">
          ...
          <section className="glass-premium rounded-3xl p-6 sm:p-8">
            ...
            {grades.length === 0 ? (
              <div className="text-[var(--t3)] text-center py-12 bg-[var(--surface-container-low)] rounded-2xl border border-[var(--border)] border-dashed">
                <MaterialIcon icon="assignment" className="text-4xl text-[var(--border)] mb-3" />
                <p className="text-sm font-medium">Report cards haven't been published yet.</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {grades.map((grade: any, i: number) => {
                  ...
                  return (
                    <div key={i} className="flex items-center justify-between p-4 border border-[var(--border)] bg-[var(--surface)] hover:border-[var(--primary)]/30 rounded-2xl transition-all shadow-sm group">
                      <div className="truncate pr-4">
                        <p className="font-bold text-[var(--t1)] text-sm mb-1 truncate">{grade.subjects?.name || "Unknown"}</p>
                        <p className="text-[10px] uppercase font-black text-[var(--t3)] tracking-widest">Score: {Math.round(grade.score)}%</p>
                      </div>
                      <div className={`w-12 h-12 rounded-xl flex items-center justify-center font-black text-lg border shadow-sm shrink-0 transition-transform group-hover:scale-105 ${getGradeColor(letter)}`}>
                        {letter}
                      </div>
                    </div>
                  );
          ...
          <section className="glass-premium rounded-3xl p-6 sm:p-8">
            <h2 className="text-lg font-black text-[var(--t1)] flex items-center gap-2 mb-6">
              <MaterialIcon icon="event_available" className="text-purple-500" />
              14-Day Attendance
            </h2>

            {attendance.length === 0 ? (
              <p className="text-sm text-[var(--t3)] italic text-center py-8">No attendance records recorded recently.</p>
            ) : (
              <div className="flex items-center gap-2 sm:gap-3 overflow-x-auto pb-4 custom-scrollbar">
                {attendance.map((day: any, i: number) => {
                  ...
                  return (
                    <div key={i} className="flex-shrink-0 text-center flex flex-col items-center">
                      <div className={`w-12 h-16 rounded-2xl flex items-center justify-center border shadow-sm transition-transform hover:-translate-y-1 ${
                        isPresent ? "bg-emerald-50 border-emerald-100 text-emerald-600" : 
                        isAbsent ? "bg-red-50 border-red-100 text-red-600" : 
                        "bg-amber-50 border-amber-100 text-amber-600"
                      }`}>
                        <MaterialIcon icon={isPresent ? "check" : isAbsent ? "close" : "schedule"} style={{fontSize: 24, fontWeight: 'bold'}} />
                      </div>
                      <span className="text-[10px] font-black uppercase tracking-widest text-[var(--t3)] mt-3">
                        {new Date(day.date).toLocaleDateString("en-UG", { weekday: "short" })}
                      </span>
