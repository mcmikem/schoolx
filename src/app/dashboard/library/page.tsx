        ...
        const { error } = await supabase
          ...
          .update({
            ...
            available_copies: Math.max(
              0,
              (editingBook.available_copies || 0) + (totalCopies - editingBook.total_copies),
            ),
          })
          ...
          <div className="flex gap-3">
            <div className="relative">
              <input
                ...
                className="pl-10 pr-4 py-3 bg-white border border-slate-200 rounded-2xl text-xs font-bold outline-none focus:ring-4 focus:ring-slate-100 w-full min-w-[200px]"
              />
            ...
            <div className="bg-white rounded-[2.5rem] w-full max-w-md max-h-[calc(100vh-1.5rem)] sm:max-h-[calc(100vh-2rem)] overflow-y-auto my-auto shadow-2xl p-8 transform animate-in slide-in-from-bottom-8 duration-300">
              <div className="flex justify-between mb-8 items-center">
                <h2 className="text-2xl font-black text-slate-800 tracking-tight">
                  {editingBook ? "Edit Volume" : "Add Volume"}
                </h2>
                <button
                  onClick={() => {
                    setShowAdd(false);
              ...
              <div className="space-y-5">
                ...
                <button
                  onClick={saveBook}
                  disabled={saving || Boolean(libraryValidationError)}
                ...
                >
                  {saving ? (
                    ...
                    <>
                      <MaterialIcon icon="save" /> {editingBook ? "Save Changes" : "Register Volume"}
                    </>
                  )}
                </button>
                {libraryValidationError && <p className="text-sm text-slate-500">{libraryValidationError}</p>}
              </div>
            </div>
          </div>
        )}
