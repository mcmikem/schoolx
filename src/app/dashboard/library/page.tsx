...
export default function LibraryPage() {
  ...
  const availableBooks = books.reduce(
    (s, b) => s + Number(b.available_copies || 0),
    0,
  );
  ...
  const handleReturnBook = async (bookId: string) => {
    ...
    try {
      const { error } = await supabase
        ...
        .eq("id", bookId)
        .eq("school_id", school.id)
        .lt("available_copies", supabase.raw("total_copies"));
      if (error) throw error;
  ...
  const handleAddVolume = async (bookId: string) => {
    if (!school?.id) return;
    try {
      const { error } = await supabase
        .from("library_books")
        .update({
          total_copies: supabase.raw("total_copies + 1"),
      ...
      toast.error(getErrorMessage(err, "Failed to add volume"));
    }
  };
        ...
        <div className="flex justify-between items-end flex-wrap gap-4">
          <div>
            ...
            <p className="text-slate-500 font-medium">
              Manage book inventory and student borrowing
            </p>
          </div>
          <div className="flex gap-3">
            <div className="relative">
              <input
                ...
                className="pl-10 pr-4 py-3 bg-white border border-slate-200 rounded-2xl text-xs font-bold outline-none focus:ring-4 focus:ring-slate-100 min-w-[280px]"
              />
              <MaterialIcon
                icon="search"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {loading ? (
            Array.from({ length: 6 }).map((_, i) => (
              <div
                key={i}
                className="h-40 bg-slate-50 rounded-3xl animate-pulse"
              />
            ))
          ) : filtered.length === 0 ? (
            <div className="col-span-full py-16 text-center">
              <MaterialIcon
                icon="auto_stories"
                className="text-4xl text-[var(--t3)] opacity-30 mb-2"
              />
            ...
            filtered.map((book) => {
              ...
              return (
                <div
                  key={book.id}
                  className="bg-white p-6 rounded-3xl border border-slate-100 hover:shadow-xl transition-all group relative overflow-hidden"
                >
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <span
                      className={`text-[10px] font-black ${book.available_copies === 0 ? "text-red-500" : "text-emerald-600"}`}
                    >
                  ...
                  <h3 className="text-sm font-black text-slate-800 mb-1 leading-snug group-hover:text-indigo-600 transition-colors uppercase tracking-tight">
                    {book.title}
                  </h3>
                  <p className="text-xs text-slate-400 font-bold mb-4 italic">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="w-full h-2 bg-slate-50 rounded-full overflow-hidden">
                      <div
                        className={`h-full rounded-full transition-all duration-700 ${pct > 50 ? "bg-emerald-400" : pct > 20 ? "bg-amber-400" : "bg-red-400"}`}
                        style={{ width: `${pct}%` }}
                      />
                    </div>
                    <div className="flex justify-between items-center pt-2">
                        ...
                        <div className="flex gap-2">
                            <button onClick={() => handleIssueBook(book.id)} className="p-2 bg-slate-50 hover:bg-indigo-600 hover:text-white rounded-xl transition-all" title="Issue Book">
                                <MaterialIcon icon="arrow_forward" style={{ fontSize: 18 }} />
                            </button>
                            <button onClick={() => handleReturnBook(book.id)} className="p-2 bg-slate-50 hover:bg-emerald-600 hover:text-white rounded-xl transition-all" title="Return Book">
                                <MaterialIcon icon="arrow_back" style={{ fontSize: 18 }} />
                            </button>
                            <button onClick={() => handleAddVolume(book.id)} className="p-2 bg-slate-50 hover:bg-amber-600 hover:text-white rounded-xl transition-all" title="Add Volume">
            ...
            <div className="bg-white rounded-[2.5rem] w-full max-w-md shadow-2xl p-8 transform animate-in slide-in-from-bottom-8 duration-300">
              <div className="flex justify-between mb-8 items-center">
                ...
                <button
                  ...
                  className="w-10 h-10 flex items-center justify-center hover:bg-slate-100 rounded-2xl transition-colors"
                >
              ...
              <div className="space-y-5">
                {(["title", "author", "isbn"] as const).map((f) => (
                  <div key={f}>
                    ...
                    <input
                      ...
                      className="w-full px-5 py-3.5 bg-slate-50 border border-slate-100 rounded-2xl font-bold text-sm outline-none focus:ring-2 focus:ring-indigo-100 transition-all placeholder:text-slate-300"
                      placeholder={`Enter ${f}...`}
                    />
                  </div>
                ))}
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <select
                      ...
                      className="w-full px-5 py-3.5 bg-slate-50 border border-slate-100 rounded-2xl font-bold text-sm outline-none focus:ring-2 focus:ring-indigo-100"
                    >
                ...
                <button
                  ...
                  className="w-full py-4 bg-indigo-600 text-white rounded-[1.5rem] font-black uppercase tracking-widest hover:shadow-xl hover:shadow-indigo-600/20 active:scale-95 transition-all disabled:opacity-50 flex items-center justify-center gap-2 mt-4"
                >
                  {saving ? (
                    <span className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                  ) : (
                    <>
                      <MaterialIcon icon="save" /> Register Volume
                    </>
                  )}
                </button>
              </div>
