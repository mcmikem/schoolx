...
const UGANDA_TERM_DATES_2026 = [
  ...
  {
    date: "2026-02-14",
    name: "Term 1 Midterm Break Starts",
    type: "holiday",
    color: "#f59e0b",
  },
  {
    date: "2026-02-16",
    name: "Term 1 Midterm Break Ends",
    type: "holiday",
    color: "#f59e0b",
  },
  {
    date: "2026-04-03",
    name: "Term 1 Closes (EOT)",
    type: "academic",
    color: "#ef4444",
  },
  {
  ...
  {
    date: "2026-06-26",
    name: "Term 2 Midterm Break Starts",
    type: "holiday",
    color: "#f59e0b",
  },
  {
    date: "2026-06-28",
    name: "Term 2 Midterm Break Ends",
    type: "holiday",
    color: "#f59e0b",
  },
  {
    date: "2026-08-14",
    name: "Term 2 Closes (EOT)",
    type: "academic",
    color: "#ef4444",
  },
  {
  ...
  {
    date: "2026-10-19",
    name: "Term 3 Midterm Break Starts",
    type: "holiday",
    color: "#f59e0b",
  },
  {
    date: "2026-10-21",
    name: "Term 3 Midterm Break Ends",
    type: "holiday",
    color: "#f59e0b",
  },
  {
    date: "2026-12-05",
    name: "Term 3 Closes (EOT)",
    type: "academic",
    color: "#ef4444",
  },
];
...
function TermCalendar({
  ...
  const allEvents = useMemo(() => {
    ...
    return [...builtIn, ...school].sort((a, b) =>
  ...
  const monthLabel = (key: string) => {
    const [y, m] = key.split("-");
    return new Date(Number(y), Number(m) - 1, 1).toLocaleDateString("en-UG", {
      month: "long",
      year: "numeric",
    });
  };
  ...
  return (
    <div className="p-4 space-y-4">
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start gap-2 flex-wrap">
                          <span className="text-sm font-medium text-[var(--t1)] leading-tight">
                            {ev.title}
                          </span>
                          {isToday && (
      ...
      {showModal && (
        ...
        >
          <div
            className="bg-[var(--surface)] rounded-2xl w-full max-w-md p-6 space-y-4"
          ...
          >
            ...
            <form onSubmit={handleSave} className="space-y-4">
              <div>
                <label className="text-xs font-medium text-[var(--t2)] mb-1 block">
                  Event Title *
                </label>
                <input
                  value={form.title}
...
export default function TimetablePage() {
  ...
  const detectConflicts = useCallback(
    (teacherId: string, day: number, periodNumber: number): string[] => {
      const found: string[] = [];
      ...
      if (teacherConflict) {
        const conflictClass = classes.find(
          (c: any) => c.id === teacherConflict.class_id,
        );
        found.push(
          `Teacher is already scheduled in ${conflictClass?.name || "another class"} at this slot`,
        );
      }
      ...
      if (classConflict) {
        found.push(
          `This slot is already occupied by ${classConflict.subjects?.name || "another subject"}`,
        );
      }
  ...
  const handleSaveTimetable = async () => {
    ...
    try {
      ...
      toast.error(getErrorMessage(err, "Failed to save timetable"));
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="p-8 max-w-7xl mx-auto space-y-6">
        ...
        <div className="border-b border-[var(--border)]">
          <Tabs
            tabs={mainTabs}
            activeTab={mainTab}
            onChange={(id) => setMainTab(id as "timetable" | "calendar")}
          />
        </div>
        ...
        <div className="flex items-center justify-between px-4 pt-4 pb-2 gap-3 flex-wrap">
          ...
          <Button onClick={handleSaveTimetable} disabled={!selectedClassId || loading} size="sm">
            <MaterialIcon icon="save" className="text-sm mr-1" /> Save Timetable
          </Button>
        </div>
              ) : (
                <select
                    ...
                    <table className="w-full border-collapse">
                      ...
                      <tbody>
                        {effectiveSlots.map((slot: any) => {
                          const isBreak =
                            slot.is_break ?? slot.is_lesson === false;
                          const entry = getEntry(
                            day.value,
                            slot.order_number ?? slot.period_number,
                          );
                          return (
                            <tr
                              key={slot.id}
                              className={
                                isBreak
                                  ? "bg-[var(--surface-container-low)]/50"
                                  : ""
                              }
                            >
                                    ...
                                    <div className="flex items-center justify-between">
                                      <div className="flex items-center gap-1.5 text-xs text-[var(--t4)]">
                                        <MaterialIcon
                                          ...
                                          className="text-sm"
                                        />
                                        {teacherNameById[entry.teacher_id] ||
                                          "Teacher"}
                                      </div>
                                      <button
                                        onClick={() =>
                ...
                <form onSubmit={handleAddEntry} className="space-y-4">
                  <div className="p-3 bg-[var(--primary)]/10 rounded-xl mb-4">
                    ...
                    <p className="text-sm text-[var(--t1)] font-medium">
                      {DAYS.find((d) => d.value === selectedDay)?.full} ·{" "}
                      {selectedSlot.name} (
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-[var(--t2)]">
                      Subject
                    </label>
                    <select
                      name="subject_id"
