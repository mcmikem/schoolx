...
function TermCalendar({ schoolId, userId }: { schoolId: string; userId: string }) {
  ...
  const handleDelete = async () => {
    ...
    const evResult = await withTimeout(supabase.from("events").delete().eq("id", id), 15000, timeoutFallback());
    const error = evResult?.error;
    if (error) {
  ...
  return (
    <>
      <div className="p-4 space-y-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-[var(--t3)]">
              Uganda public holidays, term dates, midterm breaks & school events
            </p>
          </div>
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-start gap-2 flex-wrap">
                            ...
                            {isToday && (
                              <span className="px-1.5 py-0.5 bg-[var(--primary)] text-white text-[9px] font-bold rounded uppercase tracking-wide">
                                Today
                              </span>
                            )}
                            {ev._builtin && (
                              <span className="px-1.5 py-0.5 bg-gray-100 text-gray-500 text-[9px] rounded uppercase tracking-wide">
                                Uganda Govt
                              </span>
                            )}
                          </div>
                          <div className="flex items-center gap-2 mt-0.5">
                            <span className="text-xs text-[var(--t3)]">
                              {new Date(ev.start_date + "T00:00:00").toLocaleDateString("en-UG", {
                                weekday: "short",
                                day: "numeric",
                                month: "short",
                              })}
                              {ev.end_date &&
                                ...
                                ` – ${new Date(ev.end_date + "T00:00:00").toLocaleDateString("en-UG", { day: "numeric", month: "short" })}`}
                            </span>
                            <span
                              className="px-1.5 py-0.5 text-[9px] font-medium rounded capitalize"
                        ...
                        {!ev._builtin && (
                          <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button
                              onClick={() => openEdit(ev)}
                              className="p-1 hover:bg-[var(--surface)] rounded text-[var(--t3)] hover:text-[var(--t1)]"
                            >
                              <MaterialIcon icon="edit" className="text-sm" />
                            </button>
                            <button
                              onClick={() => setPendingDeleteEventId(ev.id)}
                              className="p-1 hover:bg-red-50 rounded text-[var(--t3)] hover:text-red-500"
                            >
        ...
        {showModal && (
          <div
            className="fixed inset-0 bg-black/50 z-50 flex items-start sm:items-center justify-center overflow-y-auto p-3 sm:p-4"
            onClick={() => setShowModal(false)}
          >
            <div
              className="bg-[var(--surface)] rounded-2xl w-full max-w-md max-h-[calc(100vh-1.5rem)] sm:max-h-[calc(100vh-2rem)] overflow-y-auto p-6 space-y-4 my-auto"
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <button
                  onClick={() => setShowModal(false)}
                  className="p-1.5 hover:bg-[var(--bg)] rounded-lg text-[var(--t3)]"
                >
...
export default function TimetablePage() {
  ...
  const teacherNameById = useMemo(
    () => Object.fromEntries(teachers.map((teacher): [string, string] => [teacher.id, teacher.full_name])),
    [teachers],
  );
  ...
  const canAutoGenerate =
    !!user && ["super_admin", "school_admin", "admin", "headmaster", "dean_of_studies"].includes(user.role);
  ...
  const fetchTimetable = useCallback(async () => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .select(
          "id, class_id, subject_id, teacher_id, day_of_week, period_number, start_time, end_time, room, academic_year, subjects(name, code)",
        )
  ...
  const detectConflicts = useCallback(
    (
      ...
      const classConflict = timetable.find((t) => t.day_of_week === day && t.period_number === periodNumber);
      if (classConflict) {
      ...
      if (room && room.trim()) {
        ...
        if (roomConflict) {
          ...
          found.push(
            `Room "${room}" is already booked for ${conflictClass?.name || "another class"} at an overlapping time`,
          );
        }
      }

      // Check for overload (Scenario 20)
      const teacherPeriods = allClassTimetables.filter((t) => t.teacher_id === teacherId).length;
  ...
  return (
    <PageErrorBoundary>
      <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-6">
        <PageHeader title="Timetable" subtitle="Class schedule and academic calendar" />
        ...
        <TabPanel activeTab={mainTab} tabId="timetable">
          ...
          <Card className="overflow-hidden">
              ...
              {canAutoGenerate && (
                <Button size="sm" variant="secondary" onClick={() => setShowGenerateModal(true)}>
                  <span className="inline-flex items-center gap-2">
                    <MaterialIcon icon="auto_awesome" className="text-lg" /> Auto-generate
                  </span>
                </Button>
              )}
            </div>
            ...
            <div className="border-b border-[var(--border)]">
              <Tabs tabs={dayTabs} activeTab={selectedDay.toString()} onChange={(id) => setSelectedDay(parseInt(id))} />
            </div>
                ...
                <form onSubmit={handleAddEntry} className="space-y-4">
                  <div className="p-3 bg-[var(--primary)]/10 rounded-xl mb-4">
                    ...
                    <p className="text-sm text-[var(--t1)] font-medium">
                      {DAYS.find((d) => d.value === selectedDay)?.full} · {selectedSlot.name} (
                    ...
                    <div className="space-y-2">
                      {conflicts.map((c, i) => {
                        ...
                        return (
                          <div
                            key={i}
                            className={`p-3 ${isWarning ? "bg-amber-50 border-amber-200 text-amber-700" : "bg-red-50 border-red-200 text-red-700"} border rounded-xl flex items-start gap-2 text-sm`}
                          >
                            <MaterialIcon
                              icon="warning"
                              className={`${isWarning ? "text-amber-500" : "text-red-500"} shrink-0 mt-0.5`}
                            />
                            <span>{c}</span>
                          </div>
                        );
