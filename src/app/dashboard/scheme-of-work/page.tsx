  ...
  const loadScheme = useCallback(async () => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        const weekMap = new Map(data.map((row: any) => [Number(row.week_number ?? row.week), row]));
        setWeeks(
          baseWeeks.map((week) => {
  ...
  const saveScheme = async () => {
    ...
    try {
      ...
      if (toInsert.length > 0) {
        const result = await wt(supabase.from("scheme_of_work").insert(toInsert), 15000, tf());
      ...
      logger.error("Failed to save scheme:", err);
        ...
        {selectedClass && selectedSubject && (
          <div className="mt-6 bg-blue-50 rounded-xl p-4 border border-blue-100">
            ...
            <p className="text-xs text-[var(--t3)]">
              Fill this scheme of work at the beginning of term. It helps you plan what to teach each week and ensures
              you cover the full syllabus. Use the NCDC curriculum to identify topics and objectives for each subject.
            </p>
          </div>
        )}
      </div>
