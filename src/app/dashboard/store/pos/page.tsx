      ...
      <div className="bg-white px-6 py-4 flex justify-between items-center border-b border-slate-100 shrink-0">
        <div className="flex items-center gap-4">
           ...
           <div>
              ...
              <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest">{school?.name || 'SkoolMate Canteen'}</p>
           </div>
        </div>
        
        <div className="flex items-center gap-6">
           <div className="text-right">
              <p className="text-xs font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Terminal</p>
      ...
      <div className="flex-1 flex overflow-hidden">
                ...
                {items.filter(i => activeCategory === 'All' || i.category === activeCategory).map(item => (
                  ...
                  >
                    ...
                    <div className="absolute top-4 right-4 text-[9px] font-black uppercase text-slate-400 tracking-tighter">
                       Stock: {item.stock_quantity > 0 ? item.stock_quantity : 'OUT'}
                    </div>
                  </button>
                ))}
             </div>
        ...
        <div className="w-[400px] bg-white border-l border-slate-100 flex flex-col shrink-0 flex-1 lg:flex-none">
          ...
          <div className="p-6 border-b border-slate-100">
             <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-4">Customer Identity</h3>
             {!student ? (
                <div className="p-4 rounded-[28px] border-2 border-dashed border-slate-200 flex flex-col items-center justify-center text-center space-y-3 hover:bg-slate-50/50 transition-colors cursor-pointer group">
                   ...
                   <div>
                      <p className="text-xs font-bold text-slate-800">Scan Student ID</p>
                      <p className="text-[10px] text-slate-400 font-medium">Verify wallet balance instantly</p>
                   </div>
                   <button 
                    onClick={() => setStudent({ id: 'demo123', first_name: 'Isaac', last_name: 'Mugisha', balance: 15000, student_number: 'SM/2026/042' })}
                ...
                <div className="p-4 rounded-[28px] bg-primary-800 text-white relative overflow-hidden">
                   ...
                   <div className="flex items-center gap-3 relative z-10">
                      ...
                      <div className="min-w-0">
                         <p className="text-sm font-black truncate">{student.first_name} {student.last_name}</p>
                         <p className="text-[10px] font-black opacity-60 uppercase tracking-widest">{student.student_number}</p>
                      </div>
                      <button onClick={() => setStudent(null)} className="ml-auto p-1.5 hover:bg-white/20 rounded-lg shrink-0">
                   ...
                   <div className="mt-4 pt-4 border-t border-white/10 flex justify-between items-end relative z-10">
                      ...
                      <div className="text-right">
                         <p className="text-[9px] font-black bg-emerald-500/20 text-emerald-300 px-2 py-0.5 rounded-full uppercase tracking-tighter">Verified</p>
                      </div>
                   </div>
                </div>
             )}
                ...
                {cart.map(item => (
                  <div key={item.id} className="flex items-center gap-3 animate-in slide-in-from-right duration-300">
                     ...
                     <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[10px] text-slate-400 font-bold tracking-tighter">UGX {item.price.toLocaleString()}</p>
                     </div>
