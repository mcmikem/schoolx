...
export default function SchoolsPage() {
  ...
  const [ticketForm, setTicketForm] = useState({
    type: "bug" as SupportTicket["type"],
    title: "",
    description: "",
  ...
  const fetchPendingModuleRequests = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok || !result.success) {
        throw new Error(typeof result.error === "string" ? result.error : "Failed to load pending module requests");
      }
  ...
  const handleApproveModuleRequest = async (request: PendingModuleRequest) => {
    ...
    try {
      ...
      const result = await parseApiResponse(response);
      if (!response.ok || !result.success) {
        throw new Error(typeof result.error === "string" ? result.error : "Failed to approve module request");
      }
  ...
  const handleAddSchool = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await parseApiResponse(response);
      if (!response.ok || !result.success) {
        throw new Error(typeof result.error === "string" ? result.error : "Failed to add school");
      }

      const schoolId = ((result.data as { schoolId?: string } | undefined)?.schoolId as string) || "";
      ...
      const { error } = await supabase
        ...
        .update({
          ...
          subscription_ends_at: new Date(
            Date.now() + (subForm.billing === "annual" ? 365 : 30) * 24 * 60 * 60 * 1000,
          ).toISOString(),
        })
  ...
  const handleExtendTrial = async () => {
    ...
    try {
      ...
      const { error } = await supabase
        .from("schools")
        .update({ subscription_status: "trial", trial_ends_at: trialEnds })
        .eq("id", selectedSchool.id);
      if (error) throw error;
  ...
  const handleAddUser = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await parseApiResponse(response);
      if (!response.ok || !result.success) {
        throw new Error(typeof result.error === "string" ? result.error : "Failed to create user");
      }
  ...
  const handleSaveFeatures = async () => {
    ...
    try {
      const { error } = await supabase
        .from("schools")
        .update({ custom_features: enabledModules })
        .eq("id", selectedSchool.id);
      if (error) throw error;
  ...
  const filtered = schools.filter((s) => {
    ...
    const matchPlan = filterPlan === "all" || normalizePlan(s.subscription_plan) === filterPlan;
              ...
              >
                ...
                <div className="flex items-center justify-between text-sm mb-2">
                  <span
                    ...
                    style={{
                      backgroundColor: `${PLANS[normalizePlan(school.subscription_plan)]?.color}15`,
                      color: PLANS[normalizePlan(school.subscription_plan)]?.color,
                    }}
                  >
                ...
                <div className="mt-3 pt-3 border-t border-[#e8eaed] flex gap-3">
                  ...
                  <button
                    onClick={(e) => {
                      ...
                      setSubForm({
                        plan:
                          school.subscription_plan === "suspended"
                            ? "starter"
                            : normalizePlan(school.subscription_plan),
                        status: school.subscription_status === "suspended" ? "active" : school.subscription_status,
                        billing: "annual",
                      });
            ...
            <div className="modal max-w-3xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
              <div className="p-6 border-b border-gray-100">
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => setShowSchoolDetail(false)} className="p-2 text-gray-400 hover:text-gray-600">
                ...
                <div className="flex flex-wrap gap-2">
                  <Button
                    onClick={() => {
                      setSubForm({
                        plan:
                          selectedSchool.subscription_plan === "suspended"
                            ? "starter"
                            : normalizePlan(selectedSchool.subscription_plan),
                        status:
                  ...
                  <button
                    ...
                    className="text-sm px-4 py-2 rounded-xl border font-medium text-red-600 border-red-200 hover:bg-red-50"
                  >
                        ...
                        <div key={u.id} className="flex items-center justify-between p-3 bg-[#f8f9fa] rounded-xl">
                          ...
                          <div className="flex items-center gap-2 ml-4">
                            ...
                            <button
                              ...
                              className={`px-2 py-0.5 rounded-full text-xs font-medium ${u.is_active ? "bg-red-50 text-red-600 hover:bg-red-100" : "bg-green-50 text-green-600 hover:bg-green-100"}`}
                            >
                ...
                <div className="p-6 space-y-4">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="input"
                    >
                      <option value="starter">Starter (UGX 50K/student)</option>
                      <option value="growth">Growth (UGX 65K/student)</option>
                      <option value="enterprise">Enterprise (UGX 80K/student)</option>
                    </select>
                  </div>
                  <div className="flex gap-3 pt-4">
                    ...
                    <Button
                      type="button"
                      onClick={() => {
                        if (!newSchool.name || !newSchool.school_code || !newSchool.district) {
                ...
                <div className="p-6 space-y-4">
                  <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
                    ...
                    <p className="text-xs text-amber-700">
                      This person will manage the school. They'll use these credentials to log in.
                    </p>
                  </div>
                  <div>
                  ...
                  <div>
                    <label className="label">Admin Phone (Login ID)</label>
                    <input
                      type="tel"
                      inputMode="tel"
                  ...
                  <div className="flex gap-3 pt-4">
                    <Button type="button" onClick={() => setAddStep(1)} variant="secondary" className="flex-1">
                      Back
                    </Button>
                    <Button type="button" onClick={handleAddSchool} className="flex-1">
                      Create School & Admin
                    </Button>
                  </div>
                </div>
            ...
            <div className="modal max-w-md" onClick={(e) => e.stopPropagation()}>
              <div className="p-6 text-center">
                ...
                <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 mb-6 text-left">
                  <div className="flex items-start gap-2">
                    ...
                    <div className="text-xs text-amber-800">
                      <strong>Save these credentials!</strong> Share them with the school admin securely. They won't be
                      shown again.
                    </div>
                  </div>
                </div>
                ...
                <div className="flex gap-3">
                  <Button onClick={() => setShowCreatedModal(false)} variant="secondary" className="flex-1">
                    Close
                  </Button>
                  <Button
                    onClick={() => {
              ...
              <div className="p-6 space-y-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setSubForm({ ...subForm, billing: e.target.value })}
                    className="input"
                  >
                ...
                <div className="flex gap-3">
                  <Button onClick={() => setShowSubModal(false)} variant="secondary" className="flex-1">
                    Cancel
                  </Button>
          ...
          <div className="modal-overlay" onClick={() => setShowResetModal(false)}>
            <div className="modal" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="p-6 space-y-4">
                ...
                <div className="flex gap-3">
                  <Button onClick={() => setShowResetModal(false)} variant="secondary" className="flex-1">
                    Cancel
                  </Button>
          ...
          <div className="modal-overlay" onClick={() => setShowTrialModal(false)}>
            <div className="modal" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="p-6 space-y-4">
                ...
                <div className="flex gap-3">
                  <Button onClick={() => setShowTrialModal(false)} variant="secondary" className="flex-1">
                    Cancel
                  </Button>
                  <Button onClick={handleExtendTrial} className="flex-1">
                    Extend Trial
                  </Button>
                </div>
              </div>
          ...
          <div className="modal-overlay" onClick={() => setShowSuspendModal(false)}>
            <div className="modal" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="p-6 space-y-4">
                ...
                <div className="flex gap-3">
                  <Button onClick={() => setShowSuspendModal(false)} variant="secondary" className="flex-1">
                    Cancel
                  </Button>
                  <Button
                    onClick={handleSuspendSchool}
                    variant={selectedSchool.subscription_status === "suspended" ? "primary" : "danger"}
                    className="flex-1"
                  >
                    {selectedSchool.subscription_status === "suspended" ? "Reactivate" : "Suspend"}
                  </Button>
            ...
            <div className="modal max-w-md" onClick={(e) => e.stopPropagation()}>
              ...
              <form onSubmit={handleAddUser} className="p-6 space-y-4">
                ...
                <div className="bg-amber-50 border border-amber-200 rounded-lg p-3">
                  <div className="text-xs text-amber-800">
                    <strong>Save these credentials!</strong> Share them securely with the user.
                  </div>
                </div>
                <div className="flex gap-3">
                  <Button
