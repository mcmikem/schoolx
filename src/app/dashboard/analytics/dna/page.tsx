...
export default function PerformanceDNAPage() {
  ...
  const searchResults =
    searchStudent.length >= 2
      ? students
          .filter((s) => `${s.first_name} ${s.last_name}`.toLowerCase().includes(searchStudent.toLowerCase()))
          .slice(0, 8)
      : [];
  ...
  useEffect(() => {
    ...
    async function loadDNA() {
      try {
        ...
        if (gradesCount > 0) {
          const totalPct = grades!.reduce((sum, g) => sum + (g.max_score > 0 ? (g.score / g.max_score) * 100 : 0), 0);
        ...
        const punctualityPct = total > 0 ? Math.round(((total - lateCount) / total) * 100) : 0;
        ...
        const conductScore = Math.max(0, Math.min(100, 100 - Math.abs(conductPoints) * CONDUCT_PENALTY_PER_POINT));
        ...
        for (let i = 7; i >= 0; i--) {
          ...
          const avgGrade =
            weekGrades.length > 0
              ? Math.round(
                  weekGrades.reduce((s, g) => s + (g.max_score > 0 ? (g.score / g.max_score) * 100 : 0), 0) /
                    weekGrades.length,
                )
              : null;
          ...
          const attPct =
            weekAtt.length > 0
              ? Math.round(
                  (weekAtt.filter((a) => a.status === "present" || a.status === "late").length / weekAtt.length) * 100,
                )
              : null;
      ...
      <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-700">
        ...
        <div className={cardClassName + " p-6"}>
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            <div>
              <h2 className="text-sm font-black text-slate-400 uppercase tracking-widest">Attendance Trend by Class</h2>
              ...
              <div className={cardClassName + " p-8 flex flex-col items-center justify-center min-h-[450px]"}>
                ...
                <div className="grid grid-cols-2 gap-4 w-full mt-4">
                  ...
                  <div className="text-center">
                    ...
                    <p
                      className={`text-lg font-black ${dnaData.conduct >= 80 ? "text-emerald-500" : dnaData.conduct >= 60 ? "text-amber-500" : "text-red-500"}`}
                    >
                      {dnaData.conduct}%
                    </p>
                  </div>
            ...
            <div className="lg:col-span-3 space-y-6">
              <div className={cardClassName + " p-6"}>
                <div className="flex justify-between items-center mb-6">
                  <h3 className="text-sm font-black text-slate-400 uppercase tracking-widest">Weekly Trend</h3>
                  <div className="flex gap-4">
                    ...
                    <LineChart data={weeklyData}>
                      ...
                      <Tooltip
                        ...
                        formatter={(v: any, name: any) =>
                          [`${v}%`, name === "grades" ? "Grades" : "Attendance"] as [string, string]
                        }
                      />
                      <Line
                        type="monotone"
                ...
                <div className={cardClassName + " p-6 bg-slate-900 border-none relative overflow-hidden"}>
                  ...
                  <div className="space-y-1.5 relative z-10">
                    <p className="text-xs text-white/80 font-medium">
                      {dnaData.gradesCount} grade{dnaData.gradesCount !== 1 ? "s" : ""} recorded this term
                    </p>
                    <p className="text-xs text-white/80 font-medium">{dnaData.attendanceCount} attendance records</p>
                    {dnaData.lateCount > 0 && (
                      <p className="text-xs text-amber-300 font-medium">
                        {dnaData.lateCount} late arrival{dnaData.lateCount !== 1 ? "s" : ""}
                      </p>
                    )}
                    {dnaData.conductPoints !== 0 && (
                      <p
                        className={`text-xs font-medium ${dnaData.conductPoints < 0 ? "text-red-300" : "text-emerald-300"}`}
                      >
                        {dnaData.conductPoints > 0 ? "+" : ""}
                        {dnaData.conductPoints} conduct points
                      </p>
                    )}
                  </div>
                </div>
